...
- **A successful retry now shows a transient card instead of piling up on the timeline** — Leaving a card per retry round was meant to make retry history traceable, but several consecutive success cycles inside one turn pinned a row of cards to the timeline; in-progress and failed retries still leave a card and a toast, while **the success card clears itself** so the timeline is no longer filled with retry records (covered by a real-DOM E2E regression: only one card while retrying, gone once it succeeds).
...
- **The ask card is pinned to the bottom of the column and takes over the composer, with full-width option rows (#230)** — A blocking question no longer scrolls away with the timeline content: the card moved to the bottom of the timeline column, level with the composer, so it stays in view while you scroll back for context. While it is up the composer collapses to nothing (kept mounted rather than unmounted — the paste-to-file cleanup hangs off the composer's own buttons, so unmounting would leave orphaned temp files), and the card's height ceiling is “column height − the transcript's reserved area”, scrolling internally only when it is genuinely long. Options are full-width rows everywhere (the 2/4-column grid and its 72px minimum-height heuristic are gone), so long option text is no longer squeezed into a narrow strip; single-value answers (single select / confirm) advance to the next question automatically and submit on the last one, multi_select / input / editor still advance explicitly, and single-question batches deliberately do not auto-advance so “click the option = submit” cannot misfire (thanks @bfzha).
