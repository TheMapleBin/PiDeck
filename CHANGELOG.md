...
- **The sidebar project tree shows the current Git branch** — The project row displays the current branch (tooltip “Current branch: xxx”), so multi-worktree setups and frequent branch switches no longer require opening the Git drawer (thanks @xiaYuTian11, PR #232).
...
- **The model catalogue moves to pi 0.86.0 (1438 models)** — Refreshed from upstream: 140 entries added, 56 removed and 140 changed against the previous snapshot, with three new providers (radius at 27 entries, baseten, cloudflare-ai-gateway), amazon-bedrock gaining its apac / eu / global / in / us-gov region prefixes (+42) and a large openrouter refresh. Settings → Model catalogue → “Check for updates” pulls it right away, from either source, with no release required.
...
- **The session tab bar is no longer stretched by long titles** — Tab width caps had been widened to 208 / 256px for long titles and are now tightened to 176px (normal tabs) and 224px (tabs carrying a DSH / image-gen / Plan / Goal badge): long titles no longer fill the whole bar, while the micro font size and per-line character budget stay. The contrast problems of the tab hover tooltip and the breadcrumb workspace segment (grey, hard-to-read hover text on light themes) are fixed as well (thanks @xiaYuTian11, PR #226).
...
- **Retry status appears as a card *and* a toast** — Every retry round now leaves its own card, so retry history is traceable instead of only the last result being visible.
...
Thanks to **juzijun233, bfzha, xiaYuTian11, Q-xuan, zx3022448** and other contributors for their code 🙏

What each contributed this cycle: juzijun233 (the stability / security hardening series — no more main-process freezes from PATH probing, session and event boundaries, per-session subscription isolation), xiaYuTian11 (current Git branch in the sidebar project tree #232, timeline scroll fixes #224, session tab width and font size #226, skill rename routed by type #227), bfzha (run-control semantics split, web service loopback binding with token auth, shell text and status glyph fixes), Q-xuan (clickable inline-code file references #228), zx3022448 (external resource import #221).
