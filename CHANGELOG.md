...
- **Smaller UI polish** — Notice toasts truncate long messages with a details dialog; the sidebar "show more" row splits its counts into right-aligned columns; dock entry tooltips are unified.
...
- **Model catalog hydration slowdown** — Default hydration no longer loads extension models; the refresh button pulls them back on demand.
