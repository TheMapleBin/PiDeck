...
- **DSH sandbox runner uses local Node 24** — On Windows the sandbox runner no longer starts as electron.exe (GUI). It reuses the machine’s `node.exe` (detect / set the path in Developer settings). The installer no longer ships an extra 86MB Node binary; if none is found, PiDeck prompts you to install one and falls back to the old path (which may still flash a console).
