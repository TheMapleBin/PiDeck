...
const SOURCE_TURNS = ["scroll-anchor-source-one", "scroll-anchor-source-two", "scroll-anchor-source-three", "scroll-anchor-source-four", "scroll-anchor-source-five", "scroll-anchor-source-six", "scroll-anchor-source-seven"];
...
async function sendTurn(window: Page, prompt: string) {
	...
	await expect(window.locator(".message-timeline")).toContainText(`Mock 回复：「${prompt}」流式渲染验证完成`, { timeout: 20_000 });
}
...
async function startSlowStreamingTurn(window: Page, prompt: string, responseText = `Mock 回复：「${prompt}」`) {
	...
	await expect(sendButton).toBeEnabled();
	await sendButton.click();
	await expect(window.locator(".message-timeline")).toContainText(responseText, { timeout: 10_000 });
}
...
async function scrollThenOpenNewSession(window: Page, anchorText: string, desiredOffset: number): Promise<number> {
	return window.evaluate(
		({ targetText, requestedOffset }) => {
			const timeline = document.querySelector<HTMLElement>(".message-timeline");
			const anchor = Array.from(document.querySelectorAll<HTMLElement>("article.user-turn")).find((element) => element.textContent?.includes(targetText));
...
async function waitForAnchorRestore(window: Page, anchorText: string, expectedOffset: number) {
	await expect.poll(async () => Math.abs((await readAnchorOffset(window, anchorText)) - expectedOffset), { timeout: 5_000, intervals: [100, 250, 500] }).toBeLessThanOrEqual(28);
}

async function placeAnchorInViewport(window: Page, desiredOffset: number, anchorText = ANCHOR_TEXT): Promise<number> {
	await window.evaluate(
		({ targetText, desiredOffset: requestedOffset }) => {
			const timeline = document.querySelector<HTMLElement>(".message-timeline");
			const anchor = Array.from(document.querySelectorAll<HTMLElement>("article.user-turn")).find((element) => element.textContent?.includes(targetText));
			if (!timeline || !anchor) throw new Error("scroll anchor is not mounted");
			const currentOffset = anchor.getBoundingClientRect().top - timeline.getBoundingClientRect().top;
...
test("a reader can escape the live edge with gradual wheel scrolling while an agent streams", async ({ app, window }) => {
	...
	const timeline = window.locator(".message-timeline");
	await startSlowStreamingTurn(window, "SLOW MDEMO active-wheel-anchor", "以下是渲染元素巡检：");
