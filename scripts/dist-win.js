...
const formats = args.length > 0 ? args.join(" ") : "nsis portable zip";
...
const expectedReleaseAssets = [...requiredUpdaterAssets, `${artifactPrefix}portable.exe`, `${artifactPrefix}win.zip`];
...
if (fs.existsSync(releaseDir)) {
	...
	const missingUpdaterAssets = requiredUpdaterAssets.filter((name) => !fs.existsSync(path.join(releaseDir, name)));
