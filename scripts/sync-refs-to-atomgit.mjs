...
console.log(
  `   分支：GitHub ${ghRefs.heads.size} / AtomGit ${agRefs.heads.size}，` +
    `待补 ${branchPlan.missing.length}、待更新 ${branchPlan.differing.length}、已一致 ${branchPlan.upToDate}`
);
console.log(
  `   标签：GitHub ${ghRefs.tags.size} / AtomGit ${agRefs.tags.size}，` +
    `待补 ${tagPlan.missing.length}、待更新 ${tagPlan.differing.length}、已一致 ${tagPlan.upToDate}`
);

// ── 2. 无待推内容：按需只跑写权限探针 ─────────────────────────────────────────
const hasWork =
  branchPlan.missing.length + branchPlan.differing.length + tagPlan.missing.length + tagPlan.differing.length > 0;
...
if (!hasWork && !probeWriteOnly) {
  ...
  if (branchPlan.agOnly.length || tagPlan.agOnly.length) {
    console.log(
      `ℹ️  AtomGit 独有（仅报告，不自动删除）：${[...branchPlan.agOnly, ...tagPlan.agOnly].join('、') || '无'}`
    );
  }
  process.exit(0);
}
...
if (branchDiverged.length || tagDiverged.length) {
  const lines = [
    ...branchDiverged.map((i) => `分支 ${i.name}（AtomGit 独有提交 ${i.agSha.slice(0, 8)}，GitHub ${i.sha.slice(0, 8)}）`),
    ...tagDiverged.map((i) => `标签 ${i.name}（AtomGit ${i.agSha.slice(0, 8)}，GitHub ${i.sha.slice(0, 8)}）`),
  ];
  if (force) {
    ...
    annotate(
      'warning',
      `以下 ref 在 AtomGit 侧存在独有提交（非快进），已跳过以防覆盖平台侧贡献：\n   ${lines.join('\n   ')}\n` +
...
for (const item of mismatches) {
  console.log(`   ❌ ${shortRef(item.kind, item.name)}：复核 SHA 为 ${String(item.got).slice(0, 8)}，期望 ${String(
    (item.kind === 'tags' ? ghRefs.tags : ghRefs.heads).get(item.name)
  ).slice(0, 8)}`);
}
