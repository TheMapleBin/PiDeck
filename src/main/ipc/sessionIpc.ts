...
} from "../sessions/directorySessionImport";
...
export function registerSessionIpc(deps: SessionIpcDeps): void {
	...
	ipcMain.handle(
		ipcChannels.directorySessionsScan,
		async (_event, projectId: string, dir: unknown) => {
			const project = projectStore.get(projectId);
			if (!project) throw new Error(`Project not found: ${projectId}`);
	...
	ipcMain.handle(
		...
		async (_event, projectId: string, dir: unknown, sourcePaths: unknown) => {
			const project = projectStore.get(projectId);
			if (!project) throw new Error(`Project not found: ${projectId}`);
			const report = await directorySessionImporter.import(
				projectId,
				requireImportDirectory(dir),
				requireImportSourcePaths(sourcePaths),
			);
