...
export class PiRpcClient extends EventEmitter {
	...
	request(command: Record<string, unknown>, timeoutMs = 30_000): Promise<RpcResponse> {
		...
		if (this.closed) {
			return Promise.reject(this.closeReason ? new Error(`${this.closeReason.message} (RPC command not sent: ${String(command.type)})`) : new Error(`RPC client closed: ${String(command.type)}`));
		}
