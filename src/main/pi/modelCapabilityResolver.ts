 ...
 */
function piRuntimeCandidates(runtimeModels: readonly AvailableModel[], bundledByKey: Map<string, ModelCapabilityCandidate>): ModelCapabilityCandidate[] {
	return runtimeModels.flatMap((model) => {
		...
		const candidate: ModelCapabilityCandidate = {
			...
			...(asInput(model.input) === undefined && model.images === true ? { input: ["text", "image"] as const } : {}),
		};
 ...
 */
export function resolveModelSpecFromCatalogs(input: ModelCapabilityLookupInput, index: PiAiCatalogIndex, runtimeModels?: readonly AvailableModel[]): ModelSpec | null {
	...
	const candidates = [...(runtimeModels ? piRuntimeCandidates(runtimeModels, bundledByKey) : []), ...bundled];
 ...
 */
export function resolveModelSpecFromPiCatalogs(input: ModelCapabilityLookupInput, index: PiAiCatalogIndex): ModelSpec | null {
