...
export class SessionSummaryCache<V> {
	...
	set(filePath: string, version: SessionFileVersion, value: V): void {
		...
		// 内容与版本都相同则跳过，减少无意义写盘
		if (prev && prev.version.mtimeMs === version.mtimeMs && prev.version.size === version.size && prev.value === value) {
			return;
		}
	...
	private async loadFromDisk(): Promise<void> {
		...
		for (const [path, entry] of Object.entries(parsed.entries)) {
			if (!entry || typeof entry !== "object" || !entry.version || typeof entry.version.mtimeMs !== "number" || typeof entry.version.size !== "number") {
				continue;
			}
