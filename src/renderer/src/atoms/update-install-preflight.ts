 ...
 */
export const updateInstallPreflightTasksAtom = atom<Map<string, UpdateInstallPreflightTask>>(new Map());

/** Run every registered flush even when one fails, so one pending editor cannot hide another. */
export async function flushUpdateInstallPreflight(tasks: Iterable<UpdateInstallPreflightTask>): Promise<boolean> {
