...
import { ArrowDown, ArrowUp, Bookmark, FileText, Folder, FolderInput, FolderOpen, type LucideIcon, MoreHorizontal, Pencil, Undo2 } from "lucide-react";
...
function flattenResources(items: SidebarResource[], expanded: Set<string>, depth = 0, parentId: string | null = null): FlatResource[] {
...
function insertResource(items: SidebarResource[], resource: SidebarResource, targetId: string | null, position: SidebarResourceDropPosition): SidebarResource[] {
...
function defaultIcon(item: SidebarResource, expanded: boolean) {
	const Icon = item.kind === "folder" || item.kind === "project" ? (expanded ? FolderOpen : Folder) : item.kind === "bookmark" ? Bookmark : FileText;
...
function ResourceMenuAction({ icon: Icon, onSelect, children }: { icon: LucideIcon; onSelect: () => void; children: ReactNode }) {
	return (
		<button type="button" onClick={onSelect} className="flex h-8 w-full items-center gap-2 rounded-lg px-2.5 text-left text-xs text-foreground outline-none transition-colors hover:bg-muted focus-visible:bg-muted focus-visible:ring-2 focus-visible:ring-ring">
...
function ResourceRow({ row, active, expanded, focused, draggingId, dropTarget, menuOpen, moves, renaming, onDragEnd, onDragOver, onDragStart, onDrop, onFocus, onKeyDown, onMenuOpenChange, onRenameCancel, onRenameCommit, onRenameStart, onSelect, onToggle, renderIcon, renderMenu, setRef }: ResourceRowProps) {
	const reduce = useReducedMotion() ?? false;
	...
	const menu = renderMenu?.(row.item, {
		...
		<>
			...
			{moves.up || moves.down || moves.into || moves.out ? <div aria-hidden="true" className="my-1 h-px bg-border" /> : null}
			{moves.up ? (
...
export function AISidebar({ items, defaultItems = [], onItemsChange, onMove, onMoveError, onRename, activeId, defaultActiveId = null, onActiveChange, defaultExpandedIds = [], renderIcon, renderMenu, ariaLabel = "Resources", className }: AISidebarProps) {
	...
	const focusedRow = focusedId !== null && flat.some((row) => row.item.id === focusedId) ? focusedId : (flat[0]?.item.id ?? null);
	...
	const performMove = useCallback(
		async (move: SidebarResourceMove) => {
			...
			setAnnouncement(target ? `Moved ${moved?.label ?? "item"} ${move.position} ${target.label}.` : `Moved ${moved?.label ?? "item"} to the top level.`);
	...
	const handleKeyDown = useCallback(
		(event: KeyboardEvent<HTMLDivElement>, row: FlatResource) => {
			...
			if (row.item.disabled) {
				...
				} else if (moveModifier || ["ArrowRight", "Enter", " ", "F2", "ContextMenu"].includes(event.key) || (event.shiftKey && event.key === "F10")) {
					event.preventDefault();
				}
				return;
			}
	...
	return (
		<>
			<div
				...
				className={cn("relative flex min-w-0 flex-col gap-0.5 [overflow-anchor:none] group-data-[state=collapsed]/sidebar:hidden", draggingId && "select-none pb-9", className)}
			>
					...
					{flat.map((row) => (
						<ResourceRow
							...
							onDragOver={(event, targetRow) => {
								...
								const position = !targetRow.item.disabled && canContain(targetRow.item) && ratio >= 0.25 && ratio <= 0.75 ? "inside" : ratio < 0.5 ? "before" : "after";
