...
import { TEXT_SHIMMER_CLASS_NAME, TEXT_SHIMMER_KEYFRAMES, textShimmerStyle } from "@/lib/text-shimmer";
import { cn } from "@/lib/utils";

const DEFAULT_PHRASES = ["Thinking", "Reading the context", "Connecting the details", "Forming a response"];
...
function CascadePhrase({ phrase, reduce, shimmerDuration }: PhraseProps) {
	...
	if (reduce) {
		return (
			<span className={cn("col-start-1 row-start-1 inline-block justify-self-start whitespace-pre", TEXT_SHIMMER_CLASS_NAME)} style={textShimmerStyle(shimmerDuration)}>
				{text}
			</span>
		);
	}
	...
	return (
		<AnimatePresence initial={false}>
			<motion.span key={phrase} className="col-start-1 row-start-1 inline-block justify-self-start whitespace-pre" initial="initial" animate="animate" exit="exit">
				{text.split("").map((character, characterIndex) => (
					<motion.span
						...
						className={cn("inline-block whitespace-pre will-change-[opacity,transform]", TEXT_SHIMMER_CLASS_NAME)}
...
function SwapPhrase({ phrase, reduce, shimmerDuration }: PhraseProps) {
	return (
		<AnimatePresence initial={false}>
			<motion.span
				key={phrase}
				className={cn("col-start-1 row-start-1 inline-block justify-self-start whitespace-nowrap will-change-[opacity,transform]", TEXT_SHIMMER_CLASS_NAME)}
				style={textShimmerStyle(shimmerDuration)}
...
function ScramblePhrase({ phrase, reduce, shimmerDuration }: PhraseProps) {
	...
	return (
		<span className={cn("col-start-1 row-start-1 inline-block justify-self-start whitespace-pre font-mono tabular-nums", TEXT_SHIMMER_CLASS_NAME)} style={textShimmerStyle(shimmerDuration)}>
			{display}
		</span>
	);
}

export function ReasoningText({ phrases = DEFAULT_PHRASES, variant = "cascade", interval = 1800, shimmerDuration = 2.2, indicator, className }: ReasoningTextProps) {
	...
	const longestPhrase = safePhrases.reduce((longest, current) => (current.length > longest.length ? current : longest));
	...
	return (
		<>
			...
			<span role="status" aria-live="polite" aria-labelledby={statusId} className={cn("inline-flex items-center gap-2 text-sm font-medium text-muted-foreground", className)}>
            ...
            单字符指示器占位不变，文本指示器正常撑开。 */}
				...
				<span aria-hidden="true" className="grid overflow-hidden text-left">
					...
					{variant === "cascade" ? <CascadePhrase {...phraseProps} /> : variant === "scramble" ? <ScramblePhrase {...phraseProps} /> : <SwapPhrase {...phraseProps} />}
				</span>
