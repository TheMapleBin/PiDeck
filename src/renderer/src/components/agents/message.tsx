...
export interface MessageProps extends Omit<ComponentPropsWithRef<typeof motion.article>, "children"> {
	from: MessageFrom;
...
export function Message({ from, animateIn = false, children, className, initial, animate, transition, exit, style, ...props }: MessageProps) {
		...
		<MessageSideContext.Provider value={from === "user" ? "end" : "start"}>
			<MessageContext.Provider value={{ from }}>
				<motion.article
					...
					className={cn("group/message flex w-full items-start gap-2", from === "user" ? "flex-row-reverse" : "flex-row", className)}
					{...props}
				>
					{children}
				</motion.article>
...
export function MessageGroup({ spacing = "compact", className, ...props }: MessageGroupProps) {
	return <div data-slot="message-group" className={cn("flex w-full flex-col", spacing === "compact" ? "gap-1.5" : "gap-4", className)} {...props} />;
}

export function MessageAvatar({ placeholder = false, children, className, ...props }: MessageAvatarProps) {
	return (
		<div data-slot="message-avatar" aria-hidden={placeholder || undefined} className={cn("grid size-7 shrink-0 place-items-center overflow-hidden rounded-full bg-muted text-xs font-medium text-muted-foreground [&_img]:size-full [&_img]:object-cover [&_svg]:size-3.5", placeholder && "invisible", className)} {...props}>
			{children}
		</div>
	);
}
...
export function MessageContent({ className, ...props }: MessageContentProps) {
	...
	return <div data-slot="message-content" className={cn("flex min-w-0 flex-1 flex-col gap-1.5", from === "user" ? "items-end" : "items-start", className)} {...props} />;
}

export function MessageHeader({ className, ...props }: MessageHeaderProps) {
	const { from } = useContext(MessageContext);

	return <div data-slot="message-header" className={cn("flex items-center gap-1.5 px-1 text-[11px] leading-none text-muted-foreground", from === "user" ? "justify-end" : "justify-start", className)} {...props} />;
}

export function MessageFooter({ className, ...props }: MessageFooterProps) {
	const { from } = useContext(MessageContext);

	return <div data-slot="message-footer" className={cn("flex min-h-5 items-center gap-1 px-1 text-[11px] text-muted-foreground", from === "user" ? "justify-end" : "justify-start", className)} {...props} />;
}

export function MessageMarker({ className, ...props }: MessageMarkerProps) {
	return <div data-slot="message-marker" className={cn("mx-auto flex w-fit max-w-[88%] items-center gap-1.5 rounded-full bg-muted/70 px-2.5 py-1 text-center text-xs text-muted-foreground", className)} {...props} />;
}
