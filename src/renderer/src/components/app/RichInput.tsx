...
import { formatFilePathRef, parseRichInputChips, unwrapFileChipPath, type ComposerChip, type RichInputChip } from "../session/composer/chips";
 ...
 */
function walkFlat(root: HTMLElement, onText: (node: Text, start: number, end: number) => void, onChip: (el: HTMLElement, start: number, end: number) => void, onBreak?: (start: number, end: number) => void): void {
	let offset = 0;
...
function collectTextRuns(root: HTMLElement): TextNodeRun[] {
	...
	walkFlat(
		root,
		(node, s, e) => runs.push({ node, start: s, end: e }),
		() => {},
	);
	return runs;
}
...
function collectChipRanges(root: HTMLElement): { start: number; end: number }[] {
	...
	walkFlat(
		root,
		() => {},
		(_el, s, e) => chips.push({ start: s, end: e }),
	);
	return chips;
}

/** 纯文本偏移 → DOM Range 定位。 */
function resolveOffset(runs: TextNodeRun[], offset: number): { node: Node; offset: number } | null {
...
/** 计算光标的屏幕坐标，用于菜单锚定。 */
export function getRichInputCaretCoords(root: HTMLElement, offset: number): { top: number; left: number } {
	const runs = collectTextRuns(root);
...
function unwrapChipSpan(el: HTMLElement): void {
	...
	const prevText: Text | null = el.previousSibling?.nodeType === Node.TEXT_NODE ? (el.previousSibling as Text) : null;
	const nextText: Text | null = el.nextSibling?.nodeType === Node.TEXT_NODE ? (el.nextSibling as Text) : null;
...
export const RichInput = forwardRef<HTMLDivElement, RichInputProps>(function RichInput(props, ref) {
	const { value, onChange, onCursorChange, onKeyDown, onPaste, onDrop, onDragOver, onFocus, onBlur, disabled, placeholder, className, caretRef, onChipClick, validCommandNames, validFilePaths, validSessionRefs } = props;
	...
	const rebuildDom = useCallback(
		(restoreCaret?: number | null) => {
			...
			// 是打字场景下最准确的光标来源。
			const caret = restoreCaret ?? nativeInputCaretRef.current ?? getCaretOffset(root);
	...
	useLayoutEffect(() => {
		...
		const rangesSame = existingRanges.length === chips.length && existingRanges.every((r, i) => r.start === chips[i].start && r.end === chips[i].end);
	...
	const handleClick = useCallback(
		(event: React.MouseEvent<HTMLDivElement>) => {
			...
			const label = chip.querySelector(".input-chip__label")?.textContent ?? raw?.slice(1) ?? "";
			if (raw && kind) {
	...
	const classNames = ["rich-input", disabled && "is-disabled", className].filter(Boolean).join(" ");
