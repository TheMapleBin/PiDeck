...
import { compareStatusLetter, FileIcon, fileNameOnly, statusTone, Twistie } from "./GitResourceTree";
...
const GRAPH_COLORS = ["var(--git-graph-lane-1)", "var(--git-graph-lane-2)", "var(--git-graph-lane-3)", "var(--git-graph-lane-4)", "var(--git-graph-lane-5)", "var(--git-graph-lane-6)"];
...
function GraphLanes({ row, current }: { row: GraphRow; current: boolean }) {
	...
	const inputIndex = input.findIndex((node) => node.id === commit.hash);
	const nodeColor = inputIndex !== -1 ? input[inputIndex].color : (output[nodeIndex]?.color ?? 0);
	...
	return (
		<span className="block h-7 shrink-0 overflow-hidden" style={{ width }}>
			<svg className="block max-w-none [&_path]:[vector-effect:non-scaling-stroke] [&_circle]:[vector-effect:non-scaling-stroke]" width={width} height={GRAPH_ROW_HEIGHT} viewBox={`0 0 ${width} ${GRAPH_ROW_HEIGHT}`} aria-hidden="true">
				...
				{commit.parents.length > 1 && <circle cx={laneX(nodeIndex)} cy={GRAPH_ROW_HEIGHT / 2} r={GRAPH_NODE_RADIUS + 2} fill="var(--git-panel-bg)" stroke={GRAPH_COLORS[nodeColor % GRAPH_COLORS.length]} strokeWidth="1.6" />}
...
function GraphContinuation({ row }: { row: GraphRow }) {
	...
	return (
		<span className="block h-[26px] shrink-0 overflow-hidden" style={{ width }} aria-hidden="true">
			<svg className="block max-w-none [&_path]:[vector-effect:non-scaling-stroke] [&_circle]:[vector-effect:non-scaling-stroke]" width={width} height="26" viewBox={`0 0 ${width} 26`}>
				{row.output.slice(0, MAX_VISIBLE_GRAPH_LANES).map((node, index) => (
					<path key={`${node.id}-${index}`} d={`M ${laneX(index)} 0 V 26`} fill="none" stroke={GRAPH_COLORS[node.color % GRAPH_COLORS.length]} strokeWidth="1.6" strokeLinecap="round" />
				))}
			</svg>
		</span>
	);
...
function CommitFileRow(props: { file: GitChangedFile; row: GraphRow; onOpen: () => void | Promise<void> }) {
	const { file, row } = props;
	const [opening, setOpening] = useState(false);
	const name = fileNameOnly(file.path);
	const description = file.originalPath ? t("git.renamedFrom", { path: file.originalPath }) : file.path;
	return (
		<button
			type="button"
		...
		>
			...
			<span className="ml-[5px] flex w-4 shrink-0 justify-end text-xs font-semibold text-right text-[var(--git-desc-fg)]" aria-hidden="true">
				{opening ? <Loader2 size={13} className="animate-pideck-spin" /> : compareStatusLetter(file.status)}
			</span>
		</button>
	);
}
...
function estimateGraphDetailBytes(state: CommitDetailState): number {
	...
	const text = [commit.hash, commit.authorName, commit.authorEmail, commit.message, commit.fullMessage ?? "", ...commit.parents, ...commit.refNames];
...
function CommitHoverCard(props: { hover: CommitHoverState; state: CommitDetailState | undefined; onMouseEnter?: () => void; onMouseLeave?: () => void }) {
		...
		>
			<div className="flex min-w-0 items-start gap-2">
				...
				<span className="flex min-w-0 flex-1 flex-wrap items-baseline gap-x-[5px]">
					...
					<small className="w-full text-xs text-[var(--git-desc-fg)]">
						{relativeTime(commit.authorDate)} · {absoluteTime(commit.authorDate)}
					</small>
				</span>
			</div>
			<div className="my-2.5 border-y border-[var(--git-panel-border)] py-[9px] break-words whitespace-pre-wrap">{commit.fullMessage || commit.message}</div>
			<div className="mt-2">
				<code className="block truncate font-mono text-xs text-[var(--git-desc-fg)]">{commit.hash}</code>
				{refs.length > 0 && (
					<div className="mt-1.5 flex flex-wrap gap-1">
						{refs.map((item) => (
							<span key={item} className="max-w-full truncate rounded-sm border border-[var(--git-panel-border)] px-1.5 py-px text-xs text-[var(--git-desc-fg)]">
								{item}
							</span>
						))}
					</div>
			...
			{props.state?.loading && (
				<div className="mt-[9px] flex items-center gap-[7px] border-t border-[var(--git-panel-border)] pt-2 text-[var(--git-desc-fg)]">
					<Loader2 size={13} className="animate-pideck-spin" /> {t("git.loadingCommitDetails")}
				</div>
			)}
			{props.state?.error && <div className="mt-[9px] flex items-center gap-[7px] border-t border-[var(--git-panel-border)] pt-2 text-[var(--color-danger)]">{props.state.error}</div>}
			{shortStat && (
				<div className="mt-[9px] flex items-center gap-[7px] border-t border-[var(--git-panel-border)] pt-2 text-[var(--git-desc-fg)]">
...
export function SourceControlGraph(props: GitGraphProps) {
	...
	const updateDetailState = useCallback((hash: string, state: CommitDetailState) => {
		...
		while (detailAccessOrder.current.length > GRAPH_DETAIL_CACHE_LIMIT || totalBytes() > GRAPH_DETAIL_CACHE_BYTE_LIMIT) {
	...
	const runGitAction = useCallback(
		async (action: string, run: () => Promise<void> | void, successKey: TranslationKey) => {
	...
	}, [props.open, props.projectId, ref, resetCommitDetails, loadCount]);
	...
	return (
		<section
			...
			style={{ "--git-pane-height": `${props.height}px` } as CSSProperties}
		>
			<PaneHeader id={`${props.paneIdPrefix}-graph`} title={t("git.sourceControlGraph")} count={totalCount} open={props.open} onToggle={props.onToggle}>
				...
				/>
				<Button type="button" variant="ghost" size="icon-sm" className="size-7" title={t("common.refresh")} aria-label={t("common.refresh")} onClick={() => void load()}>
					<RefreshCw size={14} />
				</Button>
			</PaneHeader>
			{props.open && (
				<div className="flex min-h-0 flex-1 flex-col overflow-x-hidden overflow-y-auto overscroll-contain [scrollbar-gutter:stable]">
					{loading && !commits.length && (
						<div className="git-status-msg">
							<Loader2 size={14} className="animate-pideck-spin" /> {t("git.loadingCommits")}
						</div>
					)}
					...
					{commits.length > 0 && (
						<div className="min-h-0 flex-1 overflow-auto overscroll-contain [scrollbar-gutter:stable]" role="list" onScroll={dismissHover}>
							{graphRows.map((row) => {
								...
								return (
									<div key={commit.hash} className="" role="listitem">
										<button
											type="button"
										...
										>
											...
											<span className="flex min-w-0 items-baseline gap-2.5 overflow-hidden">
												...
												<span className="min-w-[48px] flex-[0_1_78px] truncate text-xs text-[var(--git-desc-fg)]">{commit.authorName}</span>
											</span>
										...
										{expanded && (
											<div className="min-w-0">
												...
												{commitFiles.map((file) => (
													<CommitFileRow key={`${file.originalPath ?? ""}-${file.path}`} file={file} row={row} onOpen={() => props.onOpenCommitFileDiff(commit, file)} />
												))}
											</div>
										)}
									</div>
					...
					{hover && <CommitHoverCard hover={hover} state={detailStates[hover.commit.hash]} onMouseEnter={handleCardMouseEnter} onMouseLeave={handleCardMouseLeave} />}
					...
					{contextMenu && (
						...
						>
							...
							<DropdownMenuContent align="start" side="bottom">
								<DropdownMenuItem disabled={contextMenuLoading !== null} onClick={() => runGitAction("cherryPick", () => props.cherryPick?.(props.projectId, contextMenu.commit.hash), "git.cherryPickSuccess")}>
									{contextMenuLoading === "cherryPick" ? <Loader2 size={14} className="animate-pideck-spin" /> : <GitBranch size={14} />}
									{t("git.cherryPick")}
								</DropdownMenuItem>
								<DropdownMenuItem disabled={contextMenuLoading !== null} onClick={() => runGitAction("revert", () => props.revert?.(props.projectId, contextMenu.commit.hash), "git.revertSuccess")}>
									{contextMenuLoading === "revert" ? <Loader2 size={14} className="animate-pideck-spin" /> : <RotateCcw size={14} />}
									{t("git.revert")}
								</DropdownMenuItem>
								<DropdownMenuSeparator />
								<DropdownMenuItem disabled={contextMenuLoading !== null} onClick={() => runGitAction("resetSoft", () => props.reset?.(props.projectId, contextMenu.commit.hash, "soft"), "git.contextMenuSuccess")}>
									{contextMenuLoading === "resetSoft" ? <Loader2 size={14} className="animate-pideck-spin" /> : null}
									{t("git.resetSoft")}
								</DropdownMenuItem>
								<DropdownMenuItem disabled={contextMenuLoading !== null} onClick={() => runGitAction("resetMixed", () => props.reset?.(props.projectId, contextMenu.commit.hash, "mixed"), "git.contextMenuSuccess")}>
									{contextMenuLoading === "resetMixed" ? <Loader2 size={14} className="animate-pideck-spin" /> : null}
									{t("git.resetMixed")}
								</DropdownMenuItem>
								<DropdownMenuItem
								...
								>
									{contextMenuLoading === "drop" ? <Loader2 size={14} className="animate-pideck-spin" /> : null}
									{t("git.dropCommit")}
								</DropdownMenuItem>
							</DropdownMenuContent>
						</DropdownMenu>
