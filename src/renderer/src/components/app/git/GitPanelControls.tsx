	...
	return (
		<div className="flex h-8 shrink-0 items-center gap-1 border-b border-border/40 bg-background px-2">
			...
			<div className="flex min-w-0 flex-1 items-center justify-end gap-0.5">
				{props.children}
				{typeof props.count === "number" && props.count > 0 && <span className="inline-flex h-5 min-w-5 shrink-0 items-center justify-center rounded-full bg-muted px-1.5 text-[11px] font-medium text-muted-foreground">{props.count}</span>}
			</div>
		</div>
	);
}
 ...
 */
export function GitCompactFilter(props: { value: string; options: { value: string; label: string }[]; onChange: (value: string) => void; ariaLabel: string; className?: string }) {
	const selected = props.options.find((option) => option.value === props.value);
	return (
		<Select value={toSelectValue(props.value)} onValueChange={(next) => props.onChange(fromSelectValue(next))}>
			<SelectTrigger
			...
			<SelectContent position="popper" align="end" className="max-w-[min(20rem,calc(100vw-16px))]">
