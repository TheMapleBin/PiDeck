...
export const WebTab = memo(function WebTab(props: WebTabProps) {
	...
	const tokenSuffix = webStatus?.running && webStatus.requiresAuth && webStatus.token ? `?token=${encodeURIComponent(webStatus.token)}` : "";
	const webAccessUrl = selectedWebAddress ? `http://${selectedWebAddress}:${webPortDraft || draft.webServicePort}${tokenSuffix}` : "";
	...
	return (
		<SettingsSection title={t("settings.webLocalService")} description={t("settings.webLocalServiceDesc")}>
			<SettingSwitchRow
				...
				description={props.webServiceChanging ? t("settings.webOpening") : t("settings.webOffDesc")}
				checked={draft.webServiceEnabled}
				disabled={props.webServiceChanging}
				onChange={(checked) => updateDraft({ webServiceEnabled: checked })}
			/>
            ...
            两列均分不再有主机列挤压/过宽问题，主机超长时 Input 内滚动 */}
				<div className="grid grid-cols-2 gap-2">
					<div className="min-w-0">
						...
						<Input value={draft.webServiceHost} readOnly className="mt-1 font-mono text-sm tabular-nums" />
					</div>
					<div className="min-w-0">
				...
				<div className="grid grid-cols-[auto_minmax(0,1fr)_auto] items-center gap-2.5 rounded-lg border border-border-subtle/70 bg-bg-muted/30 px-3 py-2.5">
					...
					<span className={cn("size-2 shrink-0 rounded-full", draft.webServiceEnabled ? "bg-[var(--color-accent)] shadow-[0_0_0_3px_color-mix(in_srgb,var(--color-accent)_12%,transparent)]" : "bg-text-tertiary shadow-[0_0_0_3px_color-mix(in_srgb,var(--color-text-tertiary)_12%,transparent)]")} />
					<div className="min-w-0">
						<strong className="block truncate text-caption font-semibold text-text-primary">http://127.0.0.1:{webPortDraft || draft.webServicePort}</strong>
						<small className="mt-0.5 block text-micro text-text-tertiary">{t("settings.localWebHint")}</small>
					</div>
					<Button variant="secondary" size="sm" disabled={!draft.webServiceEnabled} onClick={() => props.onOpenWebService(webPortDraft || String(draft.webServicePort))}>
						{t("common.open")}
					</Button>
				</div>
				<div className="flex justify-end">
					<Button variant="outline" size="sm" disabled={!draft.webServiceEnabled || props.webServiceChanging} onClick={props.onRestartWebService}>
					...
					{webQrDataUrl ? (
						<div className="flex flex-wrap items-center gap-3 pt-1">
							...
							<div className="min-w-0 flex-1">
								...
								{webStatus?.running && webStatus.requiresAuth ? <small className="mt-1 block text-micro text-text-tertiary">{t("settings.webQrTokenHint")}</small> : null}
							</div>
						</div>
					) : (
