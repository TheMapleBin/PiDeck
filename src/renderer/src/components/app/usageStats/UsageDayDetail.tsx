...
export function UsageDayDetail(props: { rows: UsageDayRow[]; costKnown: boolean }) {
	...
	return (
		<SettingsSection divided boxed={false} title={isToday ? t("usageStats.dayDetail.titleToday") : t("usageStats.dayDetail.titleDay", { date: selected })}>
			{/* 按天搜索工具栏：日历单选 + 回今日 */}
			...
			{!row ? (
				...
				<>
					<div className="usage-stats-cards">
						<DayCard label={t("usageStats.dayDetail.cards.tokens")} value={formatTokens(row.totals.tokens)} />
						<DayCard
						...
						<DayCard label={t("usageStats.dayDetail.cards.sessions")} value={String(row.totals.sessions.length)} />
					</div>
					...
					<div className="mt-4 rounded-md border border-border-subtle bg-bg-panel px-3 py-2.5">
						<div className="grid gap-4 xl:grid-cols-2">
							<div>
								<h4 className="mb-1.5 text-caption font-semibold text-text-secondary">{t("usageStats.dayDetail.modelsTitle")}</h4>
								<UsageTable headers={[t("usageStats.models.col.model"), t("usageStats.models.col.tokens"), t("usageStats.models.col.cost"), t("usageStats.models.col.turns")]} rows={row.byModel.map((m) => [m.model, formatTokens(m.tokens), formatCost(m.cost), String(m.turns)])} />
							</div>
							<div>
								<h4 className="mb-1.5 text-caption font-semibold text-text-secondary">{t("usageStats.dayDetail.projectsTitle")}</h4>
								<UsageTable headers={[t("usageStats.projects.col.project"), t("usageStats.models.col.tokens"), t("usageStats.models.col.cost"), t("usageStats.models.col.turns")]} rows={row.byProject.map((p) => [p.project, formatTokens(p.tokens), formatCost(p.cost), String(p.turns)])} />
							</div>
						</div>
					</div>
				</>
