		...
		<div className="grid max-w-3xl gap-4">
			<div className="flex items-start justify-between gap-2">
				...
				{draft.providers.length > 0 ? (
					<Button type="button" variant="ghost" size="sm" className="h-7 shrink-0" onClick={draft.providers.every((provider) => collapsedIds.has(provider.id)) ? expandAll : collapseAll}>
						{draft.providers.every((provider) => collapsedIds.has(provider.id)) ? t("config.imagegen.expandAll") : t("config.imagegen.collapseAll")}
					</Button>
				) : null}
			</div>
			{error ? <div className="rounded-sm border border-danger/20 bg-danger-soft px-3.5 py-2.5 text-control leading-relaxed text-danger">{error}</div> : null}
			{draft.providers.length === 0 ? <p className="text-control text-muted-foreground">{t("config.imagegen.empty")}</p> : null}
						...
						<Collapsible open={!collapsed} onOpenChange={() => toggleCollapsed(provider.id)}>
							...
							<div className="flex items-center gap-2 px-3">
								<CollapsibleTrigger asChild className="min-w-0 flex-1">
									<button type="button" className="flex min-w-0 flex-1 items-center gap-2 py-2.5 text-left" title={collapsed ? t("common.expand") : t("common.collapse")}>
										...
										<span className="truncate text-sm font-medium">{provider.name.trim() || t("config.imagegen.unnamedProvider")}</span>
										{/* 折叠态摘要：接口地址 + 模型数，收起后仍可一眼辨认供应商 */}
										{collapsed ? (
											<span className="min-w-0 flex-1 truncate pl-1 text-micro text-muted-foreground">
												{provider.baseUrl.trim() ? `${provider.baseUrl.trim()} · ${t("config.imagegen.modelsCount", { count: provider.models.filter(Boolean).length })}` : t("config.imagegen.modelsCount", { count: provider.models.filter(Boolean).length })}
											</span>
										) : null}
									</button>
								</CollapsibleTrigger>
							...
							<CollapsibleContent>
								<div className="grid gap-3 border-t border-border-subtle px-3 pb-3 pt-3">
									<div className="grid gap-1">
										...
										<Input id={`ig-name-${provider.id}`} value={provider.name} placeholder={t("config.imagegen.providerNamePlaceholder")} onChange={(event) => updateProvider(provider.id, { name: event.target.value })} />
									</div>
									<div className="grid gap-2 sm:grid-cols-2">
										<div className="grid gap-1">
											<Label htmlFor={`ig-url-${provider.id}`}>{t("config.imagegen.baseUrl")}</Label>
											<Input id={`ig-url-${provider.id}`} value={provider.baseUrl} placeholder="https://api.openai.com/v1" onChange={(event) => updateProvider(provider.id, { baseUrl: event.target.value })} />
										</div>
										<div className="grid gap-1">
											<Label htmlFor={`ig-key-${provider.id}`}>{t("config.imagegen.apiKey")}</Label>
											<Input id={`ig-key-${provider.id}`} type="password" autoComplete="off" value={provider.apiKey} onChange={(event) => updateProvider(provider.id, { apiKey: event.target.value })} />
										</div>
									</div>
									<div className="grid gap-1.5">
										...
										<div className="grid gap-1.5">
											{EXTRA_PARAM_KEYS.map((item) => (
												<label key={item.key} className="flex cursor-pointer items-start gap-2 rounded-md px-1 py-1 hover:bg-muted/40" title={t(item.hintKey)}>
													<Switch
														size="sm"
														className="mt-0.5"
									...
									<div className="grid gap-2">
										<div className="flex flex-wrap items-center justify-between gap-2">
											...
											<div className="flex gap-1.5">
												<Button type="button" variant="outline" size="sm" className="h-7" disabled={fetchingId === provider.id} onClick={() => void fetchModels(provider)}>
													{fetchingId === provider.id ? t("config.fetchingModels") : t("config.fetchModels")}
												</Button>
												<Button type="button" variant="outline" size="sm" className="h-7" onClick={() => updateProvider(provider.id, { models: [...provider.models, ""] })}>
													{t("config.imagegen.addModel")}
												</Button>
											</div>
										</div>
										{fetchErrorByProvider[provider.id] ? <div className="rounded-sm border border-danger/20 bg-danger-soft px-3.5 py-2.5 text-control leading-relaxed text-danger whitespace-pre-line">{fetchErrorByProvider[provider.id]}</div> : null}
										{fetched.length > 0 ? (
