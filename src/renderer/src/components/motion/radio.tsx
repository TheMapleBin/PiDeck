...
export function RadioGroup({ value, defaultValue = "", onValueChange, children, className, orientation = "vertical" }: RadioGroupProps) {
...
export function RadioGroupItem({ value, label, disabled, className, id: idProp }: RadioGroupItemProps) {
	...
	return (
		<label htmlFor={id} className={cn("inline-flex items-center gap-3", disabled ? "cursor-not-allowed" : "cursor-pointer", className)}>
			...
			>
				{selected ? <motion.span layoutId={layoutId} className="absolute inset-1 rounded-full bg-primary" transition={reduce ? { duration: 0 } : SPRING_LAYOUT} /> : null}
			</motion.button>
