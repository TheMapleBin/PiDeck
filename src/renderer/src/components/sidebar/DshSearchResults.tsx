 ...
 */
export function DshSearchResults(props: { query: string; onOpen: (projectId: string, sessionId: string) => void }) {
	const results = useDshSessionSearch(props.query);
	const records = useAtomValue(sessionRecordsAtom);
	const matched = results
		.map((result) => {
			const record = Object.values(records).find((candidate) => candidate.dshSessionId === result.sessionId);
	...
	return (
		<div className="dsh-search-results flex shrink-0 flex-col gap-1 rounded-lg border border-border-subtle bg-bg-panel/60 p-1.5">
			...
			{matched.map(({ record, snippet }) => (
				<button key={record.id} type="button" className="flex w-full min-w-0 cursor-pointer flex-col gap-0.5 rounded-md px-2 py-1.5 text-left transition-colors hover:bg-accent/50" onClick={() => props.onOpen(record.projectId, record.id)}>
					<TitleScrollText text={record.title} className="w-full flex-none text-control font-medium text-foreground" />
