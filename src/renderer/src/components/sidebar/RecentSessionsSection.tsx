...
/**
 * 最近会话区：分界线 + 段落标题（含 x/y 计数）+ 行 + 手动「加载更多」。
 *
 * 全量条数与可见条数由外部传入：段落只负责渲染，条数状态归活动页 owner（ActiveSessionsTree），
 * 避免同一份「显示多少条」在两处各存一份。
 */
export function RecentSessionsSection(props: { controller: SidebarController; actions: SidebarActions; currentSessionId?: string; rows: readonly RecentSessionRow[]; totalCount: number; visibleCount: number; onLoadMore: () => void }) {
	...
	return (
		<section aria-label={t("app.sidebarRecentSessions")} className="mt-1 flex flex-col gap-0">
			...
			<div className="mt-2 flex items-center gap-2 border-t border-border-subtle px-2 pt-2">
				<span className="text-caption font-medium text-muted-foreground">{t("app.sidebarRecentSessions")}</span>
			...
			{remainingCount > 0 && (
				<Button
					...
					variant="ghost"
					size="sm"
					className="session-more-btn h-auto min-w-0 w-auto flex-1 justify-start px-2 text-micro opacity-80 transition-opacity hover:opacity-100"
					aria-label={t("app.sidebarRecentLoadMore")}
					title={t("app.sidebarRecentLoadMoreHint", { count: remainingCount })}
