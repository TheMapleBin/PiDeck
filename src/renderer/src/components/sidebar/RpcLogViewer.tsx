...
const RpcLogRow = memo(function RpcLogRow(props: { log: RpcLogEntry; expanded: boolean; onToggle: () => void }) {
	...
	return (
		<div className="rpc-log-entry-wrap">
			<div className={`rpc-log-entry ${log.direction === "send" ? "log-send" : "log-recv"}`} onClick={onToggle} title={log.summary}>
				<time>
...
export function RpcLogViewer(props: RpcLogViewerProps) {
	...
	useEffect(() => {
		...
		void Promise.all([window.piDesktop.rpcLogs.getLive(agentId), props.loadHistory ? props.loadHistory(agentId) : Promise.resolve([])]).then(([live, history]) => {
			if (disposed) return;
	...
	return (
		<Dialog open onOpenChange={(next) => !next && onClose()}>
			<DialogContent showCloseButton={false} className={cn("flex flex-col gap-0 overflow-hidden p-0 sm:max-w-[min(1000px,92vw)] h-[min(720px,82vh)]")}>
				<DialogHeader className="flex-row items-center justify-between px-4 py-3">
					<DialogTitle className="flex items-center gap-2">
						<span>{t("rpc.title", { visible: renderedEntries.length, total: entries.length })}</span>
						{entries.length > 0 && (
							<span className={cn("inline-flex items-center gap-1 text-caption font-normal", following ? "text-text-secondary" : "text-text-tertiary")}>
				...
				<div className="rpc-log-toolbar">
					<div className="rpc-log-filter-tabs">
						<Button variant="outline" size="sm" className={`h-auto px-2 py-1 text-caption shadow-none${directionFilter === "all" ? " active" : ""}`} onClick={() => setDirectionFilter("all")}>
							{t("rpc.filterAll")}
						</Button>
						<Button variant="outline" size="sm" className={`h-auto px-2 py-1 text-caption shadow-none${directionFilter === "send" ? " active" : ""}`} onClick={() => setDirectionFilter("send")}>
							{t("rpc.filterSend")}
						</Button>
						<Button variant="outline" size="sm" className={`h-auto px-2 py-1 text-caption shadow-none${directionFilter === "recv" ? " active" : ""}`} onClick={() => setDirectionFilter("recv")}>
							{t("rpc.filterReceive")}
						</Button>
					</div>
					{loggingOn === true && props.setLogging && (
						<Button variant="outline" size="sm" className="h-auto px-2 py-1 text-caption shadow-none" onClick={() => void handleDisableLogging()}>
							{t("rpc.disableLogging")}
						</Button>
					)}
					<Input value={keyword} onChange={(e) => setKeyword(e.target.value)} placeholder={t("rpc.searchPlaceholder")} className="h-auto min-w-0 flex-1 border-[var(--rpc-log-border)] bg-[var(--rpc-log-input-bg)] px-2.5 py-1.5 text-caption text-[var(--rpc-log-strong)]" />
					<Button variant="outline" size="sm" className={`rpc-log-autoscroll h-auto px-2 py-1 text-caption shadow-none${autoScroll ? " active" : ""}`} onClick={() => setAutoScroll((current) => !current)} title={t("rpc.autoScroll")}>
						{t("rpc.autoScroll")}
					</Button>
				</div>

				{!hasActiveFilter && entries.length > WINDOW_UNFILTERED && <div className="border-b border-[var(--rpc-log-border)] px-4 py-1.5 text-caption text-text-tertiary">{t("rpc.windowHint", { count: WINDOW_UNFILTERED })}</div>}
				{loggingOn === false && (
				...
				<div className="relative min-h-0 flex-1">
					<MessageScroller className="h-full" followOutput={autoScroll} followThreshold={56} onFollowChange={setFollowing} scrollApiRef={scrollApiRef} label={t("rpc.title", { visible: renderedEntries.length, total: entries.length })} viewportClassName="rpc-log-list" smooth>
						{renderedEntries.map((log) => (
							<RpcLogRow key={log.id} log={log} expanded={expandedId === log.id} onToggle={() => setExpandedId((current) => (current === log.id ? null : log.id))} />
						))}
