...
export function DshPluginSection() {
	...
	const dshSessions = useMemo(() => Object.values(records).filter((record) => record.backend === "dsh" && record.dshSessionId), [records]);
			...
			{dynamicPlugins.length === 0 ? (
				...
				<div className="grid gap-1.5">
					{dynamicPlugins.map((plugin) => (
						<div key={plugin.pluginId} className="rounded-md border border-border-subtle bg-bg-panel px-2.5 py-2">
							<div className="flex items-center gap-2">
								<span className="min-w-0 flex-1 truncate text-control font-medium text-foreground">{plugin.packages[0]?.name || plugin.pluginId}</span>
								<span className={`shrink-0 rounded-full border px-2 py-0.5 text-micro ${plugin.activeRun ? "border-emerald-300/70 bg-emerald-500/10 text-emerald-700 dark:border-emerald-700/70 dark:text-emerald-300" : "border-border-subtle text-muted-foreground"}`}>
							...
							<div className="mt-0.5 flex items-center gap-2 text-caption text-text-secondary">
								...
								<span className="min-w-0 truncate">
									{t("config.dsh.pluginSession")}: {sessionTitle(plugin.agentId)}
								</span>
								{plugin.status && (
									<>
										<span className="shrink-0">·</span>
							...
							{plugin.error && (
								<p className="mt-1 truncate text-micro text-danger" title={plugin.error}>
									{plugin.error}
								</p>
							)}
...
export function PluginInventoryView() {
	...
	const filtered = useMemo(() => entries.filter((entry) => matchesEntry(entry, normalizedQuery) && (originFilter === "all" || (entry.origin ?? "builtin") === originFilter)), [entries, normalizedQuery, originFilter]);
	...
	const uninstallUserPlugin = async (entry: DshStaticPluginView) => {
		...
		try {
			...
			if (result.fileError) {
				...
				showNotice(result.keptPluginDir ? `${t("config.dsh.pluginFilesKeepHint")}：${result.keptPluginDir}` : t("config.dsh.pluginFilesKeepHint"), 8000);
			}
			try {
	...
	return (
		<div className="grid gap-2.5">
			...
			<div className="flex items-center gap-2">
				...
				<Button type="button" variant="ghost" size="sm" className="ml-auto h-6 gap-1 px-2 text-muted-foreground" onClick={() => void load()}>
					...
					{pageRows.map((entry) => {
						...
						return (
							<li key={entry.entryId} className="overflow-hidden rounded-lg border border-border-subtle bg-bg-panel">
								<button type="button" className="flex w-full items-center gap-2 px-3 py-2.5 text-left" aria-expanded={open} aria-controls={detailId} onClick={() => setExpandedId(open ? null : entry.entryId)}>
									...
									<span className={`size-1.5 shrink-0 rounded-full ${phaseDotClass(entry.fiberPhase, entry.enabled)}`} role="img" aria-label={phaseLabel(entry.fiberPhase)} title={phaseLabel(entry.fiberPhase)} />
			...
			{totalPages > 1 && <Pagination page={pageClamped} totalPages={totalPages} onPageChange={setPage} className="py-1" />}
		</div>
	);
}

/** 安装表单：选择归属会话 + 语义前缀 + 名称/用途 + Host 源码（define，不运行）。 */
function InstallPluginForm(props: { sessions: Array<{ sessionId: string; title: string }>; onInstalled: () => void }) {
	...
	return (
		<div className="grid gap-2 rounded-md border border-border-subtle bg-bg-panel p-2.5">
			...
			<div className="grid gap-1.5">
				...
				<Textarea className="min-h-24 font-mono text-caption" value={hostCode} onChange={(event) => setHostCode(event.target.value)} placeholder="export default { apply(ctx) { ... } }" />
