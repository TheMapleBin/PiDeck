...
import {
  modelKey,
  pickCycleModel,
  pickCycleThinkingLevel,
  resolveFavoriteCycleCandidates,
  type CycleDirection,
} from "../utils/preferenceCycle";
...
export function useSessionPreferenceController(options: {
   ...
   */
  const cycleModel = useCallback(async (direction: CycleDirection = "forward") => {
    const currentKey = resolvedLiveModel.provider && resolvedLiveModel.modelId
      ? modelKey(resolvedLiveModel.provider, resolvedLiveModel.modelId)
      : undefined;
   ...
   */
  const cycleThinking = useCallback(async (direction: CycleDirection = "forward") => {
