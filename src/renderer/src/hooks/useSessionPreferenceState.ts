...
import {
  resolveThinkingPickerLevels,
} from "../components/session/sessionPickerOptions";
...
import {
  ...
  type ModelPending,
} from "../utils/modelPendingDisplay";
...
export function useSessionPreferenceState(options: {
  ...
  useEffect(() => {
    ...
    if (
      !(options.thinkingPickerOpen || options.cycleArmed) ||
      isDshSession ||
      runtime?.status !== "idle" ||
      report === null ||
  ...
  const currentModelEntry = models.find(
    (model) => model.provider === resolvedLiveModel.provider && model.id === resolvedLiveModel.modelId,
  );
  const runtimeThinkingTarget = !isDshSession && runtimeLive && runtime?.agentId &&
    typeof runtime.runtimeGeneration === "number" &&
    resolvedLiveModel.provider && resolvedLiveModel.modelId
    ? {
