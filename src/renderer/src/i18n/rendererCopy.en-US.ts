...
export const enUS: Record<TranslationKey, string> = {
  ...
  "directoryImport.emptyDesc":
    "Try the pi sessions root (~/.pi/agent/sessions), one of its group folders (like --D--work-old--), or the project directory itself.",
  ...
  "directoryImport.filteredDesc":
    "The list is filtered to sessions whose original folder is gone. These ones are already visible in the sidebar, so importing them is usually unnecessary.",
  ...
  "config.strictToolSamplingDesc": "Since pi 0.86, read/bash/edit/write are sent as strict JSON-schema tools (on by default under openai-completions). A relay that does not honor strict may let tool calls leak into plain text (DeepSeek-family models emit DSML marker blocks); choose Off to fall back to pi 0.85 behavior.",
...
"config.proxyUrlUnset": "No proxy URL configured",
  ...
  "environment.guideAlreadyInstalledDesc":
    "If pi is installed but not detected (common with nvm, mise, pnpm, etc.), just paste its path into “Set pi path manually” below — no need to reinstall anything.",
  ...
  "environment.guideNodeDesc":
    "No system Node.js detected. Install a portable Node with one click (about 30 MB, lives inside PiDeck's data directory — no admin rights, no system changes).",
  ...
  "environment.guideNpmDesc":
    "npm ships with Node.js and usually needs no extra work. Once it checks out, continue to the next step.",
  ...
  "environment.guidePiDesc":
    "Installs the pi CLI into PiDeck's data directory via npm (no admin rights needed). Run the check again afterwards to start using pi.",
