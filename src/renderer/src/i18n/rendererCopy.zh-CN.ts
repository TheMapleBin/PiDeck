...
"config.proxyUrlUnset": "未配置代理 URL",
  ...
  "settings.dshRunnerNodeDesc": "Windows 上 DSH 沙箱命令需要本机 Node 24 的 node.exe（控制台程序），用来避免 electron.exe 弹出黑窗口。留空则自动检测 PATH 与常见安装位置。不随安装包分发。",
  ...
  "settings.dshRunnerNodeUseDetected": "使用已检测到的 Node",
  "settings.dshRunnerNodeMissingHint": "未找到 Node 24。请安装 https://nodejs.org/ 后在开发设置里指定 node.exe，否则 DSH 沙箱命令可能闪一下黑窗口。",
