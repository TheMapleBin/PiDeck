...
export const zhCN = {
  ...
  "directoryImport.emptyDesc":
    "可以选 pi 的 sessions 目录（~/.pi/agent/sessions）、其中一个分组目录（形如 --D--work-old--），或项目目录本身。",
  ...
  "directoryImport.filteredDesc":
    "当前按「只看原目录已失效」过滤。这批会话已经能在侧栏看到，一般不需要再导入。",
...
"config.proxyUrlUnset": "未配置代理 URL",
  ...
  "environment.guideAlreadyInstalledDesc":
    "如果 pi 已安装但未被检测到（常见于 nvm、mise、pnpm 等版本管理器环境），直接在下方「手动指定 pi 路径」填入路径即可，无需重复安装。",
  ...
  "environment.guideNodeDesc":
    "未检测到系统 Node.js。可一键下载安装便携 Node（约 30 MB，安装到 PiDeck 数据目录，不需要管理员权限，不影响系统环境）。",
  ...
  "environment.guideNpmDesc":
    "npm 随 Node.js 一起安装，通常无需单独操作。检测通过后即可进行下一步。",
  ...
  "environment.guidePiDesc":
    "使用 npm 安装 pi CLI 到 PiDeck 数据目录（不需要管理员权限）。安装后重新检测即可开始使用。",
