...
const { trimHistoryMessages, turnTrimStartIndex, countRoleMessagesBefore, isDefaultAgentTitle, inferTitleFromMessages, looksLikePiSessionFileStem } = loadTsCommonJs("src/main/pi/agentUtils.ts");
...
test("isDefaultAgentTitle treats draft placeholder titles as default so first prompt can auto-rename", () => {
	...
	assert.equal(
		isDefaultAgentTitle("New session", project, (key) => (key === "session.newTitle" ? "New session" : translateTitle(key))),
		true,
	);
	...
	assert.equal(isDefaultAgentTitle("2026-08-08T10-47-19-239Z_abc", project, translateTitle), true);
