...
const { createDevEnvironment, getElectronViteInvocation, withDefaultElectronArgs } = require("../scripts/dev.js");
...
test("runs electron-vite dev and forwards extra arguments", () => {
	...
	assert.deepEqual(invocation.args, ["/repo/node_modules/electron-vite/bin/electron-vite.js", "dev", "--debug", "main", "--", "--trace-warnings", "--ozone-platform=x11", "--log-level=3"]);
});
...
test("does not override an explicit Chromium log level", () => {
	...
	assert.deepEqual(invocation.args, ["/repo/node_modules/electron-vite/bin/electron-vite.js", "dev", "--", "--log-level=1", "--ozone-platform=x11"]);
});
