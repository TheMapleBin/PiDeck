...
test("禁用扩展提示：只有设置成因带「去设置」动作，回退只解释本次运行", () => {
	...
	for (const copy of [setting, fallback]) {
		...
		assert.ok(copy.noticeDurationMs >= 6000, `notice duration too short: ${copy.noticeDurationMs}`);
	}
});

test("禁用扩展文案的中英文词条齐备（缺词条时 toast 会直接显示 key）", () => {
	const mainZh = readFileSync("src/shared/i18n/mainProcessCopy.ts", "utf8");
	const rendererZh = readFileSync("src/renderer/src/i18n/rendererCopy.zh-CN.ts", "utf8");
	const rendererEn = readFileSync("src/renderer/src/i18n/rendererCopy.en-US.ts", "utf8");
	for (const reason of ["setting", "fallback"]) {
		const copy = resolveDisabledExtensionsCopy(reason);
		assert.match(mainZh, new RegExp(`"${copy.diagnosticKey.replaceAll(".", "\\.")}"`));
		assert.match(rendererZh, new RegExp(`"${copy.noticeKey.replaceAll(".", "\\.")}"`));
		assert.match(rendererEn, new RegExp(`"${copy.noticeKey.replaceAll(".", "\\.")}"`));
	}
