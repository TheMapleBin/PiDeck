...
const { markFileTreeLoadFailed, mergeFileTreeChildren } = loadTsCommonJs(
...
test("接线：restoreExpandedDirs 只补拉「已展开 + 无 children + 未标失败」目录", () => {
  const app = readFileSync("src/renderer/src/App.tsx", "utf8");
  const fn = app.match(/const restoreExpandedDirs = useCallback\(async \(projectId: string\): Promise<boolean> => \{[\s\S]*?\n  \}, \[beginFileTreeRequest, isFileTreeRequestCurrent\]\);/);
