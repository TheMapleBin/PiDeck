...
test("refreshAheadBehind 成功后把结果写入缓存", () => {
	const refreshBlock = source.slice(source.indexOf("const result = await aheadBehind(projectId)"), source.indexOf("    } catch {", source.indexOf("const result = await aheadBehind(projectId)")));
