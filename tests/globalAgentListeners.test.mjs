...
const source = readFileSync("src/renderer/src/hooks/useGlobalAgentListeners.ts", "utf8");
const app = readFileSync("src/renderer/src/App.tsx", "utf8");
const bootstrap = readFileSync("src/renderer/src/components/app/AppBootstrap.tsx", "utf8");
const runtimeBridge = readFileSync("src/renderer/src/hooks/useSessionRuntimeBridge.ts", "utf8");
const updateWatch = readFileSync("src/renderer/src/hooks/useBackgroundUpdateWatch.ts", "utf8");

test("project change events re-read presence-aware inventory before replacing atoms", () => {
	// 块边界不能假设 4 空格缩进（formatter 统一为 tab）：用行首缩进无关的 }); 锚点。
	...
	// 链式调用被格式化拆行（desktopApi.projects 与 .list 分行）：容忍换行。
	assert.match(changedBlock[0], /desktopApi\s*\.?\s*projects\s*\.?\s*list\(\)/);
...
test("global listener owner handles non-runtime application events only", () => {
	for (const listener of ["projects.onChanged", "pet.onFocusTarget", "projects.onTrustRequest", "settings.onApplyWindow", "app.onOpenInBrowser"]) {
...
test("Session runtime bridge is the sole runtime event and inventory owner", () => {
	// 链式调用可能被格式化拆行：容忍 sessions 与 .listRuntimes 之间的换行。
	assert.match(runtimeBridge, /sessions\s*\.?\s*listRuntimes\(\)/);
