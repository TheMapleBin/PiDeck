...
const { setI18nLocale, translateI18nDescriptor } = loadTsCommonJs("src/renderer/src/i18n.ts");
const { mainProcessT, normalizeMainProcessLocale } = loadTsCommonJs("src/shared/i18n/mainProcessCopy.ts");
...
test("main-process copy resolves locale and interpolates stable product text", () => {
	...
	assert.equal(mainProcessT("en-US", "session.historyTitle", { project: "PiDeck" }), "PiDeck history");
	assert.equal(mainProcessT("en-US", "session.imagePlaceholder"), "[Image]");
	assert.equal(mainProcessT("zh-CN", "session.importedPreview", { source: "Codex" }), "已导入的 Codex 会话");
	assert.equal(mainProcessT("en-US", "session.inUseDeleteBlocked", { title: "Review" }), 'Session "Review" is in use. Close its Agent before deleting it.');
	assert.equal(mainProcessT("zh-CN", "wsl.connectionFailed"), "无法连接到所选 WSL 发行版和用户，请检查配置后重试。");
	assert.match(mainProcessT("en-US", "wsl.piNotInstalled"), /@earendil-works\/pi-coding-agent/);
});
...
test("ConfigManager localizes product errors and does not return unknown provider exceptions", async () => {
	...
	try {
		const manager = new ConfigManager(dir, (key, params) => mainProcessT("en-US", key, params));
		...
		const provider = await manager.fetchProviderModels("https://provider.example", "secret-key", "openai-completions");
		assert.equal(provider.error, "Failed to load the model list. Check the provider configuration and try again.");
...
test("main-process managers use the injected locale for user-visible validation", async () => {
	...
	await assert.rejects(new PromptManager(tmpdir(), en).create({ name: "", description: "description" }), /template name is required/i);
	await assert.rejects(new ProjectResourceManager(() => ({ id: "project", name: "Project", path: tmpdir(), lastOpenedAt: 1 }), en).createSkill({ projectId: "project", name: "", description: "description", locationId: "project-pi" }), /lowercase letters, numbers, and hyphens/i);
	assert.equal((await new PiLocator(en).validateCustomPath("")).error, "Enter the path to pi.cmd or pi.");
	assert.equal(
		(
			await testPiProxy(
				{
...
test("structured main-process copy translates parameters and preserves fallback behavior", () => {
	...
	assert.equal(translateI18nDescriptor({ i18nKey: "missing.key" }, "fallback"), "fallback");
...
test("diagnostic renderer localizes copy while keeping raw details separate", () => {
	const diagnosticCard = readFileSync("src/renderer/src/components/session/TimelineEventCards.tsx", "utf8");
...
test("main-process Session IPC errors expose stable copy and retain diagnostics separately", () => {
	const { SessionCommandIpcError } = loadTsCommonJs("src/main/sessions/SessionCommandIpcError.ts");
...
test("main-process user surfaces use stable copy and keep caught details in logs", () => {
	...
	const appUpdateCard = readFileSync("src/renderer/src/components/app/settings/AppUpdateCard.tsx", "utf8");
