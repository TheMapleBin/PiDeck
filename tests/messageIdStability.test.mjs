...
const { stabilizeProjectedIdsFromIdentities, stabilizeReloadedMessageIds } = loadTsCommonJs("src/main/pi/historyMessages.ts");
...
test("压缩重载：同一条消息沿用事件版 id（渲染层 key 稳定，不 remount）", () => {
	...
	const previous = [runtimeMessage("画一只猫", "user"), runtimeMessage("好的，我来画")];
	const projected = [projectedMessage("画一只猫", "e1", "user"), projectedMessage("好的，我来画", "e2")];
...
test("压缩摘要卡（投影新增、无旧匹配）保持投影 id", () => {
	...
	const projected = [summaryCard, projectedMessage("画一只猫", "e1", "user"), projectedMessage("好的，我来画", "e2")];
	const stabilized = stabilizeReloadedMessageIds(previous, projected);
...
test("同文本高频消息（连发「继续」）一一消耗，不串位", () => {
	const previous = [runtimeMessage("继续", "user"), runtimeMessage("继续", "user")];
	const projected = [projectedMessage("继续", "e1", "user"), projectedMessage("继续", "e2", "user")];
	const stabilized = stabilizeReloadedMessageIds(previous, projected);
	assert.deepEqual(
		stabilized.map((m) => m.id),
		[previous[0].id, previous[1].id],
...
test("时间差远超容差 → 视为不同消息，不沿用旧 id", () => {
	const previous = [{ ...runtimeMessage("继续", "user"), timestamp: 1_000_000 }];
	const projected = [{ ...projectedMessage("继续", "e1", "user"), timestamp: 2_000_000 }];
	const stabilized = stabilizeReloadedMessageIds(previous, projected);
...
test("幂等：旧缓存已是投影版时重载不改变 id", () => {
	const previous = [projectedMessage("画一只猫", "e1", "user"), projectedMessage("好的，我来画", "e2")];
	const projected = [projectedMessage("画一只猫", "e1", "user"), projectedMessage("好的，我来画", "e2")];
	const stabilized = stabilizeReloadedMessageIds(previous, projected);
	assert.deepEqual(
		stabilized.map((m) => m.id),
...
test("压缩归档的旧消息（投影中已不存在）自然消失，不影响其余消息", () => {
	...
	const previous = [runtimeMessage("最早的问题", "user"), runtimeMessage("好的，我来画"), runtimeMessage("正在生成图片，请稍候…")];
	const projected = [projectedMessage("好的，我来画", "e2"), projectedMessage("正在生成图片，请稍候…", "e3")];
	const stabilized = stabilizeReloadedMessageIds(previous, projected);
...
test("身份摘要按 entryId 还原投影 id（重启后窗口沿用旧 key）", () => {
	const identities = [retainedIdentity("agent-1-history-e1", "e1", "user", "画一只猫")];
...
test("live 身份（无 entryId）按指纹 + 时间窗回退匹配（重启前未落投影的用户消息）", () => {
	...
	const identities = [retainedIdentity("7daf81fe-a2a9-4fc9-88be-e09995b6fe43", undefined, "user", "看下这个文档")];
...
test("entryId 匹配不做时间二次校验：正文改写/久远捕获仍沿用同一 key", () => {
	...
	const identities = [retainedIdentity("stale-id", "e1", "user", "继续", 1_000_000)];
...
test("指纹回退（无 entryId 的 live 身份）时间超容差 → 保持投影 id", () => {
	const identities = [retainedIdentity("live-stale", undefined, "user", "继续", 1_000_000)];
	const projected = [projectedMessage("继续", "e1", "user")];
	const stabilized = stabilizeProjectedIdsFromIdentities(identities, projected);
...
test("同一身份只能消耗一次：同文本两条投影只还原第一条", () => {
	...
	const identities = [retainedIdentity("live-1", undefined, "user", "继续", 2_000_000)];
	const projected = [projectedMessage("继续", "e1", "user"), projectedMessage("继续", "e2", "user")];
	const stabilized = stabilizeProjectedIdsFromIdentities(identities, projected);
...
test("同一文件条目多次捕获时后捕获优先（UI 当前展示的 id 胜出）", () => {
	const identities = [retainedIdentity("agent-1-history-e1", "e1", "user", "画一只猫"), retainedIdentity("agent-2-history-e1", "e1", "user", "画一只猫")];
	const projected = [projectedMessage("画一只猫", "e1", "user")];
	const stabilized = stabilizeProjectedIdsFromIdentities(identities, projected);
