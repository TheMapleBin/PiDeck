...
const { findModelCapabilityMatch, modelCapabilityMatchToSpec, normalizeModelIdentity, parseThinkingLevelMap } = loadTsCommonJs("src/main/pi/modelCapabilityMatch.ts");
...
test("parseThinkingLevelMap accepts only Pi's finite map and preserves explicit null", () => {
	assert.deepEqual(JSON.parse(JSON.stringify(parseThinkingLevelMap({ off: null, high: "high", bogus: "x", max: "" }))), { off: null, high: "high" });
	assert.equal(parseThinkingLevelMap({ bogus: "x" }), undefined);
});

test("exact model id resolves across a third-party provider", () => {
	const match = findModelCapabilityMatch({ providerName: "luna-relay", modelId: "gpt-5.6" }, [candidate()]);
...
test("unique proxy alias adopts the longest canonical model family", () => {
	const match = findModelCapabilityMatch(
		...
		[candidate({ id: "gpt-5", contextWindow: 272000 }), candidate()],
	);
...
test("name matching rejects embedded, ambiguous, and lower-version candidates", () => {
	assert.equal(findModelCapabilityMatch({ providerName: "relay", modelId: "mygpt56luna" }, [candidate()]), undefined);
	assert.equal(findModelCapabilityMatch({ providerName: "relay", modelId: "GPT-5.6 Luna" }, [candidate({ id: "gpt-5" })]), undefined);
	assert.equal(findModelCapabilityMatch({ providerName: "relay", modelId: "alpha-1 beta-22 relay" }, [candidate({ id: "alpha-1" }), candidate({ id: "beta-22" })]), undefined);
});
...
test("vision variants are not aliased from text-only base models", () => {
	...
	assert.equal(findModelCapabilityMatch({ providerName: "relay", modelId: "deepseek-v4-flash-vision-exp" }, [base]), undefined);
	assert.equal(findModelCapabilityMatch({ providerName: "relay", modelId: "qwen2.5-vl-72b" }, [candidate({ id: "qwen2.5", input: ["text"] })]), undefined);
});

test("same-family vision variants may still alias each other", () => {
	// -vision-exp → -vision 是同类视觉变体，能力声明一致，允许匹配。
	const match = findModelCapabilityMatch({ providerName: "relay", modelId: "deepseek-v4-flash-vision-exp" }, [candidate({ id: "deepseek-v4-flash-vision", input: ["text", "image"] })]);
	assert.equal(match?.matchKind, "name-alias");
