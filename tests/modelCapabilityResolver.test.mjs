...
const { resolveModelSpecFromPiCatalogs, resolveModelSpecFromCatalogs } = loadTsCommonJs("src/main/pi/modelCapabilityResolver.ts");
...
test("exact model id resolves across a third-party provider from bundled catalog", () => {
	const index = buildPiAiCatalogIndex([{ provider: "openai", id: "gpt-5.6", contextWindow: 400000 }]);

	const spec = resolveModelSpecFromPiCatalogs({ providerName: "luna-relay", modelId: "gpt-5.6" }, index);
...
test("unknown model resolves to null instead of guessing", () => {
	const index = buildPiAiCatalogIndex([{ provider: "openai", id: "gpt-4o", contextWindow: 128000 }]);

	const spec = resolveModelSpecFromPiCatalogs({ providerName: "luna-relay", modelId: "luna-pro-2026-beta" }, index);
...
test("runtime model missing from bundled catalog resolves with pi-reported capacity", () => {
	...
	const index = buildPiAiCatalogIndex([{ provider: "opencode-go", id: "qwen3.8-max-preview", contextWindow: 1000000, maxTokens: 131072 }]);
	const runtime = [{ provider: "opencode-go", id: "qwen3.8-max", contextWindow: 1000000, maxTokens: 131072, reasoning: true }];

	const spec = resolveModelSpecFromCatalogs({ providerName: "opencode-go", modelId: "qwen3.8-max" }, index, runtime);
...
test("runtime capacity wins over bundled for same provider+id (pi uses its own resolved values)", () => {
	const index = buildPiAiCatalogIndex([{ provider: "zai1", id: "glm-5.3", contextWindow: 1000000, maxTokens: 131072, reasoning: true }]);
	// 用户 provider 在 models.json/auth.json 里把容量改小 → 运行中 pi 实报为准
	const runtime = [{ provider: "zai1", id: "glm-5.3", contextWindow: 128000, maxTokens: 16384, reasoning: true }];

	const spec = resolveModelSpecFromCatalogs({ providerName: "zai1", modelId: "glm-5.3" }, index, runtime);
...
test("runtime candidate without capacity inherits bundled capacity for same provider+id", () => {
	...
	const index = buildPiAiCatalogIndex([{ provider: "qwen", id: "qwen3.7-max", contextWindow: 1000000, maxTokens: 131072 }]);
	const runtime = [{ provider: "qwen", id: "qwen3.7-max" }];

	const spec = resolveModelSpecFromCatalogs({ providerName: "qwen", modelId: "qwen3.7-max" }, index, runtime);
...
test("no runtime models → bundled-only fallback (same as resolveModelSpecFromPiCatalogs)", () => {
	...
	const spec = resolveModelSpecFromCatalogs({ providerName: "luna-relay", modelId: "gpt-4o" }, index, undefined);
