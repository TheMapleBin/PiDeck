...
const { parsePiListModels, parseTokenSize, modelsFromPiConfig, isUnknownCliOption, classifyModelListFailure, MODEL_LIST_FAST_ARGS, MODEL_LIST_EXT_ARGS, MODEL_LIST_COMPAT_ARGS } = loadTsCommonJs("src/main/pi/modelListCache.ts");
...
test("parsePiListModels parses table with provider/model/thinking", () => {
	const stdout = ["provider  model  context  max-out  thinking  images", "openai    gpt-5   200K     64K      yes       yes", "deepseek  v4-flash 1M     384K      yes       no"].join("\n");
...
test("parsePiListModels keeps provider names containing spaces (regression: grok.weishiair.de copy)", () => {
	...
	const stdout = ["provider  model  context  max-out  thinking  images", "grok.weishiair.de         grok-4.5                  500K     128K     yes       yes", "grok.weishiair.de copy    grok-4.6                  500K     128K     yes       yes"].join("\n");
	const models = parsePiListModels(stdout);
	assert.equal(models.length, 2);
...
test("parsePiListModels ignores unknown-option banners mixed into stdout", () => {
	const stdout = ["error: unknown option '--offline'", "provider  model  thinking", "openai    gpt-4o  no"].join("\n");
	const models = parsePiListModels(stdout);
	assert.equal(models.length, 1);
	assert.equal(models[0].id, "gpt-4o");
});
...
test("modelsFromPiConfig flattens settings-page models.json", () => {
	const models = modelsFromPiConfig({
		providers: {
			deepseek: {
				models: [{ id: "v4-flash", name: "Flash", reasoning: true, contextWindow: 1_000_000, input: ["text"] }, { id: "bad" }],
			},
			openai: {
...
test("startup prefetch hydrates capabilities after WSL configuration", () => {
	// 链式调用可能被格式化拆行：容忍 syncWslConfig() 与 .then 之间的换行。
...
test("welcome page explicit model/thinking selections persist and are promoted into the first session", () => {
	...
	const bootstrap = readFileSync("src/renderer/src/utils/chatSessionBootstrap.ts", "utf8");
...
test("model list report channel: manual refresh reruns list models with failure classification", () => {
	...
	// 共享通道集中定义
	assert.match(readFileSync("src/shared/ipc.ts", "utf8"), /projectsListModelsReport: "projects:list-models-report"/);
	...
	const plainSection = systemIpc.slice(systemIpc.indexOf("projectsListModels,"), systemIpc.indexOf("projectsListModelsReport"));
...
test("save models verification excludes config-fallback (empty name proven via local read)", () => {
	...
	assert.match(systemIpc, /report\.source === "config-fallback" \? "config-fallback" : report\.reason/);
	// 归一化侧同步根治：空 name 在写盘前被剥离，不再产出非法 models.json。
	assert.match(readFileSync("src/main/config/ConfigManager.ts", "utf8"), /normalized\.name\.length === 0/);
});
...
test("save models returns instantly (no pi fork on the save path) and verifies in background", () => {
	...
	assert.match(readFileSync("src/shared/ipc.ts", "utf8"), /configModelsVerifyResult: "config:models-verify-result"/);
