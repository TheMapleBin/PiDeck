...
const { isModelCatalogStale, refreshModelCatalogIfStale, MODEL_CATALOG_REFRESH_ARGS, MODEL_CATALOG_STALE_MS } = loadTsCommonJs("src/main/pi/modelListCache.ts");
...
test("pi update --models entry: args are array form (spawn safety) and startup wiring", () => {
	...
	assert.ok(refreshIndex >= 0 && hydrateIndex > refreshIndex && watcherIndex > hydrateIndex, "startup must finish catalog refresh and initial hydration before installing the watcher");
