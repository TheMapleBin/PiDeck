...
const CLI_TABLE = ["provider  model  context  max-out  thinking  images", "openai    gpt-5   200K     64K      yes       yes"].join("\n");
...
test("save verification (retryOnEmpty:false) forks pi exactly once even when CLI returns empty", async () => {
	...
	const deps = makeDeps();
	const report = await resolveModelListReport(deps.piLocator, deps.settingsStore, deps.configSource, true, { retryOnEmpty: false });
...
test("default behavior still retries once on empty (startup cold-start protection kept)", async () => {
	...
	const deps = makeDeps();
	const report = await resolveModelListReport(deps.piLocator, deps.settingsStore, deps.configSource, true);
