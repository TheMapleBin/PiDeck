...
const {
  ...
  filterVisibleModels,
  moveModelItem,
} = loadTsCommonJs("src/renderer/src/config/modelVisibility.ts");
...
test("契约完整性：ComposerComponents 与 ComposerPickerHost 支持 hiddenModels 并可恢复", () => {
  ...
  assert.ok(host.includes("setHiddenModels(settings.hiddenModels ?? []);"));
  assert.ok(host.includes("toggleHideModel"));
