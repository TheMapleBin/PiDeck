...
test("dialog onOpenChange handlers invoke onClose instead of returning the callback", () => {
	const files = ["components/overlays/OverlayComponents.tsx", "features/feedback/FeedbackDialog.tsx", "components/app/ImportModals.tsx", "components/app/ProjectResourcesModal.tsx", "components/session/WorkspaceSurface.tsx"];
	for (const file of files) {
		const source = read(file);
		// 禁止 `!next && onClose` / `!next && props.onClose` 这种未调用写法
		assert.doesNotMatch(source, /!next\s*&&\s*(?:props\.)?onClose(?!\s*\()/, `${file} must call onClose() in onOpenChange`);
		assert.match(source, /onOpenChange=\{\(next\)\s*=>\s*!next\s*&&\s*(?:props\.)?onClose\(\)\}/, `${file} must wire onOpenChange to onClose()`);
	}
});
...
test("runtime responder rolls back when binding changes after claim", async () => {
	...
	const responder = createSessionRuntimeUiResponder({
		...
		readBinding: () => {
			const next = { ...current };
			current = { ...current, runtimeGeneration: 3 };
			return next;
		},
		claim: () => true,
		rollback: () => {
...
test("allowOther renders a custom input and sends its value through the responder envelope", async () => {
	...
	const props = {
		...
		responder: {
			respond: async (nextRequest, response) => {
				sent.push({ nextRequest, response });
				return true;
			},
		},
	};
