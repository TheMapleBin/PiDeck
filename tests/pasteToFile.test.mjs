...
test("onPaste：大段纯文本（≥阈值）preventDefault 并转文件，不进编辑器", () => {
	...
	const plainTextBranch = controller.indexOf('event.clipboardData.getData("text/plain")');
	...
	assert.ok(plainTextBranch >= 0 && thresholdBranch >= 0 && pasteTextToFileBranch >= 0, "大文本转文件分支应存在（getData → 阈值判断 → pasteTextToFile）");
	assert.ok(plainTextBranch > imageBranch, "大文本判断应在位图分支之后（位图优先语义不变）");
});

test("右键粘贴：大段文本同样转文件并返回 true（不再交给编辑器原样插入）", () => {
	assert.match(controller, /desktopApi\.clipboard\.readText\(\)/);
	assert.match(controller, /text\.length >= PASTE_TO_FILE_MIN_CHARS/, "右键粘贴菜单（无 ClipboardEvent）也应有阈值分支");
