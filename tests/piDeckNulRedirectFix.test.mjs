...
const { normalizeNulRedirects } = loadTsCommonJs("resources/extensions/pi-deck-nul-redirect-fix.ts", {
	stubs: {
...
test("非 Windows 平台透传（临时改写 process.platform）", () => {
	...
	try {
		for (const cmd of ["echo hello > nul", "echo hello 2>> nul", "echo hello &>> nul", "echo hello >& nul"]) {
