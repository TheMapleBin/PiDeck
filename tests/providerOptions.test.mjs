...
test("默认供应商候选聚合 providers + auth + discovered 三处来源", () => {
	const options = collectProviderOptions({ providers: { tr: {}, opencode: {} } }, { bailu: {} }, { shangtang: [{ id: "sensenova-6.7-flash-lite" }] });
	assert.deepEqual(
...
test("providers 含 tokendance 时正常聚合（写入配置后经既有链路出现）", () => {
	const options = collectProviderOptions({ providers: { tokendance: {} } }, undefined, undefined);
	assert.deepEqual([...options.map((o) => o.value)], ["tokendance"]);
});

test("复现：仅 discovered 存在的供应商必须在候选里（漏掉即「无匹配选项」）", () => {
	const options = collectProviderOptions({ providers: { tr: {} } }, undefined, { shangtang: [{ id: "m1" }] });
	assert.ok(
...
test("同名供应商去重（三处来源都出现只保留一个候选）", () => {
	const options = collectProviderOptions({ providers: { tr: {} } }, { tr: {} }, { tr: [{ id: "m1" }] });
