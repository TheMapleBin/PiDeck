...
test("usagePercent：periods 封顶 100；credits remaining 可反推 used", () => {
	...
	assert.equal(display.usagePercent(creditsResult({ total: 200, remaining: 60 })), 70, "used = total - remaining = 140 → 70%");
...
test("formatUsageBadgeText：periods/credits-windows 出百分比，balance 出余额，credits 出剩余", () => {
	...
	assert.equal(display.formatUsageBadgeText(creditsResult({ windows: [{ key: "fiveHour", total: 100, used: 45 }] })), "45%");
	// 无可展示数值 → null（调用方不渲染）
...
test("usageBadgeSegments：balance 出「余额+金额」；credits 出「剩+剩余」或「已用+已用」", () => {
	const t = (key) => `#${key}`;
	const json = (value) => JSON.stringify(value);
	assert.equal(json(display.usageBadgeSegments(balanceResult(6.58, "CNY"), t)), json([{ labelKey: "config.usage.balanceShort", text: "¥6.58", tone: "ok" }]));
	assert.equal(json(display.usageBadgeSegments(creditsResult({ remaining: 120.5 }), t)), json([{ labelKey: "config.usage.remainingShort", text: "120.5", tone: "ok" }]));
	// 只有已用量：已用标签 + 中性灰
...
test("usageBadgeSegments：credits windows 逐窗出「窗口名+百分比」，剩余可反推已用", () => {
	...
	assert.equal(
		json(
			display.usageBadgeSegments(
				creditsResult({
					windows: [{ key: "fiveHour", total: 100, used: 45 }, { key: "weekly", total: 100, remaining: 90 }, { key: "customGateway", remaining: 3.5 }, { key: "emptyWindow" }],
				}),
				t,
			),
		),
	...
	assert.equal(json(display.usageBadgeSegments(creditsResult({ windows: [{ key: "emptyWindow" }], remaining: 12 }), t)), json([{ labelKey: "config.usage.remainingShort", text: "12", tone: "ok" }]));
});
...
test("relativeTimeParts：刚刚/分钟/小时/天/过期 五档（cc-switch Clock 行语义）", () => {
	...
	assert.equal(json(display.relativeTimeParts(now - 3 * 60_000, now)), json({ key: "config.usage.timeMinutesAgo", params: { n: 3 } }));
	assert.equal(json(display.relativeTimeParts(now - 2 * 3_600_000, now)), json({ key: "config.usage.timeHoursAgo", params: { n: 2 } }));
	assert.equal(json(display.relativeTimeParts(now - 5 * 86_400_000, now)), json({ key: "config.usage.timeDaysAgo", params: { n: 5 } }));
