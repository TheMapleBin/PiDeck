...
test("parseUsageResponseBody percent 非数字时退化 raw", () => {
	const res = probe.parseUsageResponseBody({ usage: { monthly: { percent: "68%" } } }, "RAW");
	assert.equal(res.matched, false);
...
test("buildProbeHeaders noBearer 时不自动补 Bearer，显式 Authorization 仍保留", () => {
	...
	assert.equal(j(probe.buildProbeHeaders({ Cookie: "sid=1; tok=2" }, "sk-1", { noBearer: true })), j({ Cookie: "sid=1; tok=2" }));
	// noBearer 不影响显式 Authorization（调用方显式声明时以显式为准）
	assert.equal(j(probe.buildProbeHeaders({ Authorization: "Bearer custom" }, "sk-1", { noBearer: true })), j({ Authorization: "Bearer custom" }));
...
test("parseUsageResponseBody 解析 balance 形态", () => {
	const res = probe.parseUsageResponseBody({ balance_infos: [{ currency: "CNY", total_balance: "110.00" }] }, "{}", { kind: "balance", valuePath: "balance_infos[0].total_balance", currencyPath: "balance_infos[0].currency" });
	assert.equal(res.matched, true);
...
test("parseUsageResponseBody balance 取不到数值时退化 raw", () => {
	const res = probe.parseUsageResponseBody({ balance_infos: [] }, "RAW", { kind: "balance", valuePath: "balance_infos[0].total_balance" });
	assert.equal(res.matched, false);
	assert.equal(res.raw, "RAW");
});

test("parseUsageResponseBody 解析 credits 形态，remaining 自动 total-used 反推", () => {
	const res = probe.parseUsageResponseBody({ data: { total_credits: 100, total_usage: 42.5 } }, "{}", { kind: "credits", totalPath: "data.total_credits", usedPath: "data.total_usage" });
	assert.equal(res.matched, true);
...
test("parseUsageResponseBody credits 显式 remainingPath 优先", () => {
	const res = probe.parseUsageResponseBody({ data: { total: 100, used: 42.5, remaining: 60 } }, "{}", { kind: "credits", totalPath: "data.total", usedPath: "data.used", remainingPath: "data.remaining" });
...
test("OpenRouter 候选解析真实 /api/v1/key 响应，remaining 直接用 API 给的值", () => {
	...
	const res = probe.parseUsageResponseBody({ data: { limit: 100, usage: 25.5, limit_remaining: 74.5, limit_reset: "monthly" } }, "{}", cand.parse);
	assert.equal(res.matched, true);
	assert.equal(res.kind, "credits");
	assert.equal(res.credits.total, 100);
...
test("OpenRouter 免费层 limit/limit_remaining 为 null 时仍命中（仅 used）", () => {
	const cand = probe.USAGE_PROBE_CANDIDATES.find((c) => c.baseUrlContains?.includes("openrouter.ai"));
	const res = probe.parseUsageResponseBody({ data: { limit: null, usage: 3.2, limit_remaining: null } }, "{}", cand.parse);
...
test("Kimi For Coding 候选：命中 api.kimi.com，解析真实 /usages 响应", () => {
	...
	const res = probe.parseUsageResponseBody({ usage: { limit: "2048", used: "214", remaining: "1834", resetTime: "2026-01-09T15:23:13Z" } }, "{}", kimi.parse);
	assert.equal(res.matched, true);
	assert.equal(res.kind, "credits");
...
test("Kimi For Coding：字段漂移下只给 limit+remaining 或 limit+used 都能解析", () => {
	...
	const onlyRemaining = probe.parseUsageResponseBody({ usage: { limit: "100", remaining: "74" } }, "{}", kimi.parse);
	...
	const onlyUsed = probe.parseUsageResponseBody({ usage: { limit: "100", used: "40" } }, "{}", kimi.parse);
...
test("Moonshot 候选国内/国际 baseUrl 都命中，解析真实 balance 响应（无币种）", () => {
	...
	assert.equal(probe.candidateApplies(cand, "https://api.openai.com/v1", "openai-completions"), false);
	const res = probe.parseUsageResponseBody({ code: 0, data: { available_balance: 12.34, voucher_balance: 2.0, cash_balance: 10.34 } }, "{}", cand.parse);
	assert.equal(res.matched, true);
	assert.equal(res.kind, "balance");
...
test("通用 OpenAI /usage 候选解析真实 /usage 响应（balance+unit）", () => {
	const generic = probe.USAGE_PROBE_CANDIDATES.find((c) => c.path === "/usage" && c.apiTypes);
	const res = probe.parseUsageResponseBody({ balance: 1.69525969, unit: "USD", planName: "钱包余额", remaining: 1.69525969 }, "{}", generic.parse);
	assert.equal(res.matched, true);
	assert.equal(res.kind, "balance");
...
test("智谱响应缺周窗条目时 windows 只给 5h 一条，主值仍正常", () => {
	const zhipu = probe.USAGE_PROBE_CANDIDATES.find((c) => c.baseUrlContains?.includes("open.bigmodel.cn"));
	const res = probe.parseUsageResponseBody({ data: { limits: [{ type: "CREDIT_LIMIT", unit: 3, number: 5, usage: 2000, currentValue: 1145 }] } }, "{}", zhipu.parse);
	assert.equal(res.matched, true);
...
test("Kimi booster 缺 balance 时 booster 不输出（主额度仍正常）", () => {
	const kimi = probe.USAGE_PROBE_CANDIDATES.find((c) => c.baseUrlContains?.includes("api.kimi.com"));
	const res = probe.parseUsageResponseBody({ usage: { limit: "100", used: "10" } }, "{}", kimi.parse);
	assert.equal(res.matched, true);
...
test("xAI billing 专用解析：percent 优先，否则 cent-wrapper 美元桶，含按需用量", () => {
	...
	const usdRes = probe.parseUsageResponseBody({ config: { monthlyLimit: { val: 3000 }, used: { val: 750 } } }, "{}", xai.parse);
...
test("preflight 子结构 URL 生成：absoluteUrl 优先、普通 path 走版本化拼接", () => {
	...
	const xai = probe.USAGE_PROBE_CANDIDATES.find((c) => c.baseUrlContains?.includes("api.x.ai"));
	const preflightUrls = probe.usageProbeUrls({ path: xai.preflight.path, absoluteUrl: xai.preflight.absoluteUrl, rootPath: false }, "https://api.x.ai", ensureVersion);
	...
	const plain = probe.usageProbeUrls({ path: "/v1/user", rootPath: false }, "https://api.x.ai", ensureVersion);
...
test("buildProbeFailureDetail 全量 401/403 归纳「鉴权」提示", () => {
	const attempts = [{ url: "https://88api.ai/api/user/self", method: "GET", status: 401, body: '{"code":"AUTH_UNAUTHORIZED"}' }];
	const detail = probe.buildProbeFailureDetail(attempts, TR);
...
test("Command Code 缺 monthlyCredits 时不匹配（关键字段缺失）", () => {
	const cc = probe.USAGE_PROBE_CANDIDATES.find((c) => c.baseUrlContains?.includes("api.commandcode.ai"));
	assert.equal(probe.parseUsageResponseBody({ windowLimits: { fiveHour: { used: 1, cap: 2 } } }, "{}", cc.parse).matched, false);
});
