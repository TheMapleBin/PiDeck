...
test("submitAgentRename updates the Session record, shows a toast, and refreshes sessions", async () => {
	...
	const api = createDefaultApi({
		renameAgent: async (id, name) => {
			calls.push(`renameAgent:${id}:${name}`);
			return agentTab({ id: "agent-1", title: name });
		},
		upsertAgent: (tab) => {
			calls.push(`upsertAgent:${tab.title}`);
		},
		showToast: (msg) => {
			calls.push(`toast:${msg}`);
		},
		refreshProjectSessions: async (pid) => {
			calls.push(`refresh:${pid}`);
		},
	});
	const harness = createRenameHarness(i18n);
	...
	assert.deepEqual(calls, ["renameAgent:agent-1:Old Name", "toast:Renamed!", "refresh:proj-a"]);
});
...
test("submitSessionRename calls renameSession API, showToast, and refreshProjectSessions", async () => {
	...
	const api = createDefaultApi({
		renameSession: async (id, name) => {
			calls.push(`renameSession:${id}:${name}`);
		},
		showToast: (msg) => {
			calls.push(`toast:${msg}`);
		},
	...
	assert.equal(r2.sessionRenameTarget, null);
	assert.equal(r2.agentRenameValue, "");
	assert.deepEqual(calls, ["renameSession:session-1:My Session", "refresh:proj-a", "toast:Renamed!"]);
});
...
test("renameModalsProps.rename.onSubmit dispatches to correct submitter", async () => {
	...
	const api = createDefaultApi({
		renameAgent: async () => {
			agentRenameCalled = true;
			return agentTab({ title: "ok" });
		},
		renameSession: async () => {
			sessionRenameCalled = true;
		},
	});
	const harness = createRenameHarness(i18n);
...
test("submitProjectRename 调用 renameProject、回写列表并 toast", async () => {
	...
	const api = createDefaultApi({
		renameProject: async (id, name) => {
			...
			return [{ id, name, path: "/tmp", lastOpenedAt: Date.now() }];
		},
		applyRenamedProjects: (projects) => {
			calls.push(`applied:${projects[0].name}`);
		},
		showToast: (msg) => {
			calls.push(`toast:${msg}`);
		},
	...
	assert.equal(r2.agentRenameValue, "");
	// 空白折叠后提交
	assert.deepEqual(calls, ["renameProject:proj-a:内部 平台", "applied:内部 平台", "toast:Renamed!"]);
});
