...
test("resizable handle maps v4 aria-orientation opposite to group layout", () => {
	...
	assert.match(resizableCode, /aria-\[orientation=horizontal\]:h-px[\s\S]*aria-\[orientation=horizontal\]:w-full/);
	// 禁止把 horizontal 当成竖线（这是导致 composer 拖不动的根因写法）
	assert.doesNotMatch(resizableCode, /aria-\[orientation=horizontal\]:h-full/);
	assert.doesNotMatch(resizableCode, /aria-\[orientation=vertical\]:h-px/);
...
test("session and shell use v4 Group orientation props only", () => {
	...
	assert.match(appShellCode, /<\/ResizablePanelGroup>[\s\S]*\{outlineContent\}/);
	assert.doesNotMatch(appShellCode, /\{outlineContent\}[\s\S]*ResizableHandle[\s\S]*shell-panel-drawer/);
});
