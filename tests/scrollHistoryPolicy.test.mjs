...
test("late history page after go-bottom may write cache but must not expand window", () => {
	...
	assert.equal(policy.shouldApplyDelayedHistoryResult({ following: true, generationAtRequest: 3, currentGeneration: 3 }), false);
	// 浏览代数已过期（回底/切会话递增代数）：不扩窗
	assert.equal(policy.shouldApplyDelayedHistoryResult({ following: false, generationAtRequest: 3, currentGeneration: 4 }), false);
	// 正常浏览中到达且代数未变：允许扩窗
	assert.equal(policy.shouldApplyDelayedHistoryResult({ following: false, generationAtRequest: 3, currentGeneration: 3 }), true);
});

test("policy types are exported for the controller to consume", () => {
	const source = readFileSync("src/renderer/src/hooks/timeline/scrollHistoryPolicy.ts", "utf8");
