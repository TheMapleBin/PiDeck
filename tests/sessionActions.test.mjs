...
const drawerSource = readFileSync("src/renderer/src/components/workspace/DrawerSurface.tsx", "utf8");

// 源码定位必须缩进无关：formatter（biome，chore/formatter-baseline）统一为 tab 后，
// 原先硬编码的两空格前缀会让 indexOf 全部返回 -1，导致整组断言连锁失败。
// 用行首锚点 + 任意空白匹配，既保留「函数体定位」语义，也不再绑定具体缩进。
function findFunctionStart(name, isAsync) {
	const pattern = isAsync ? new RegExp(`^[\\t ]*async function ${name}\\(`, "m") : new RegExp(`^[\\t ]*function ${name}\\(`, "m");
	const match = pattern.exec(source);
	return match ? match.index : -1;
}

function functionBlock(name, nextName) {
	const start = findFunctionStart(name, true);
	const end = findFunctionStart(nextName, true);
...
function syncFunctionBlock(name, nextName) {
	const start = findFunctionStart(name, false);
	const syncEnd = findFunctionStart(nextName, false);
	const asyncEnd = findFunctionStart(nextName, true);
	const end = syncEnd === -1 ? asyncEnd : asyncEnd === -1 ? syncEnd : Math.min(syncEnd, asyncEnd);
	assert.notEqual(start, -1, `${name} implementation should be discoverable`);
	assert.notEqual(end, -1, `${nextName} boundary should be discoverable`);
	return source.slice(start, end);
}
...
const commitSelection = () => syncFunctionBlock("commitSessionSelection", "selectProject");
const selectProject = () => syncFunctionBlock("selectProject", "selectSession");
const selectSession = () => syncFunctionBlock("selectSession", "copySession");
const openBySummary = () => functionBlock("openSidebarSession", "openSidebarSessionById");
const openById = () => functionBlock("openSidebarSessionById", "copySidebarSession");
const createDraft = () => {
	const start = findFunctionStart("createSessionDraft", true);
	// return 块边界同样不能假设缩进宽度，用「行首空白 + return {」匹配。
	const endMatch = /^[\t ]*return \{/m.exec(source.slice(start >= 0 ? start : 0));
	const end = endMatch ? (start >= 0 ? start : 0) + endMatch.index : -1;
...
test("accepts a cached session only when it belongs to the requested project", () => {
	...
	assert.equal(block.match(/getSessionRecord\(session\.id\)/g)?.length, 1, "the cached record should be read once");
	assertInOrder(block, ["const cachedRecord = getSessionRecord(session.id);", "cachedRecord?.projectId === projectId", "? cachedRecord", ": getProjectSessionRecords(projectId).find(", "if (!record)", "await refreshProjectSessions(projectId, true)"], "project identity fallback");
});
...
test("keeps request sequencing and stale-result gates around catalog fallback", () => {
	...
	assertInOrder(
		block,
		["const requestSequence = ++openSessionRequestRef.current;", "const cachedRecord = getSessionRecord(session.id);", "await refreshProjectSessions(projectId, true);", "if (requestSequence !== openSessionRequestRef.current) return undefined;", "record = getProjectSessionRecords(projectId).find("],
		"catalog stale gate",
	);
	assert.match(block, /catch \(error\) \{\s*if \(requestSequence !== openSessionRequestRef\.current\) return undefined;\s*showToast/);
	assert.match(block, /if \(!record \|\| requestSequence !== openSessionRequestRef\.current\) return undefined;\s*commitSessionSelection/);
});
...
test("matches both project and catalog paths with each candidate environment", () => {
	const block = openBySummary();
	const environmentMatches = block.match(/isSameSessionPath\(\s*candidate\.filePath,\s*session\.filePath,\s*candidate\.environment,?\s*\)/g);

	assert.equal(environmentMatches?.length, 2, "project and catalog fallbacks must preserve WSL path semantics");
});

test("selectProject invalidates requests before replacing the active selection", () => {
	assertInOrder(selectProject(), ["++openSessionRequestRef.current;", "commitSessionSelection(projectId, undefined, false);"], "project selection command");
	assertInOrder(commitSelection(), ["setActiveProjectId(projectId);", "setCurrentSessionId(sessionId);"], "project selection commit");
});

test("selectSession invalidates requests and can preserve the current scroll setting", () => {
	assertInOrder(selectSession(), ["++openSessionRequestRef.current;", "commitSessionSelection(projectId, sessionId, scrollToEnd);"], "session selection command");
	assert.match(commitSelection(), /scrollToEnd/);
	...
	assert.doesNotMatch(source, /\bsetAutoScroll\b|\bautoScrollRef\b/, "legacy auto-scroll ports must not leak into useSessionActions");
});
...
test("open paths commit after the stale gate without invalidating again", () => {
	for (const [name, block] of [
		...
		assertInOrder(block, ["if (!record || requestSequence !== openSessionRequestRef.current) return undefined;", "commitSessionSelection(projectId, record.id, true);"], `${name} stale gate and selection`);
		assert.equal(block.match(/\+\+openSessionRequestRef\.current/g)?.length, 1, `${name} should invalidate only when opening starts`);
	}
});
...
test("focuses the composer after publishing a new draft selection", () => {
	...
	assertInOrder(block, ["upsertSession(session);", "commitSessionSelection(projectId, session.id, true);", "creatingSessionDraftRef.current.delete(projectId);"], "draft selection and focus");
});

test("uses only the canonical project refresh port", () => {
	assert.match(source, /export type RefreshProjectSessions = \(\s*projectId: string,\s*silent\?: boolean,?\s*\) => Promise<SessionSummary\[\] \| SessionRecord\[\] \| undefined>;/);
	...
	assert.doesNotMatch(source, /\b(?:setProjectMenu|setSessionHistoryLoading|setSessionLoadingByProject)\b/);
...
test("history drawer copies the Catalog record by stable Session ID", () => {
	assert.match(drawerSource, /runCopySession\(\s*session\.id,\s*files\.sessionsProjectId/);
	assert.doesNotMatch(drawerSource, /runCopySession\(\s*session\.filePath/);
});
...
test("archive and delete dismiss the session tree and close tabs before dropping state", () => {
	...
	assertInOrder(dismiss, ["closeTabs(ids)", "removeSessionState", "removeSessionComposerState"], "dismissSessionTree must close tabs before dropping session state");
