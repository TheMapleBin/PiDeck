	...
	const context = {
		...
		require(request) {
			...
			if (request === "../atoms/session-selectors") {
				return {
					sessionRecordToSummary: (session) =>
						session.filePath
							? {
									id: session.id,
...
test("initial and collision callers share the final pending completion", async () => {
	...
	const completedBeforeQuietCycle = await Promise.race([first.then(() => true), new Promise((resolve) => setTimeout(() => resolve(false), 10))]);
	assert.equal(completedBeforeQuietCycle, false);

	finalScan.resolve([makeRecord("final", 40)]);
	const results = await Promise.all([first, collision, collisionDuringPending, collisionDuringLatest]);
	assert.equal(requestCount, 4);
	...
	assert.deepEqual(
		afterCompletionResult.map((session) => session.id),
		["final"],
	);
});
...
test("collision propagates a pending scan error", async () => {
	...
	const [firstError, collisionError] = await Promise.all([first.catch((error) => error), collision.catch((error) => error)]);
	assert.equal(requestCount, 2);
...
test("initial failure and collision share a successful pending retry", async () => {
	...
	assert.equal(requestCount, 2);
	assert.deepEqual(
		firstResult.map((session) => session.id),
		["pending"],
	);
	assert.deepEqual(
		collisionResult.map((session) => session.id),
		["pending"],
	);
});
...
test("project completions and pending retries remain isolated", async () => {
	...
	assert.deepEqual(
		projectTwoResult.map((session) => session.id),
		["project-2"],
	);
	...
	const [projectOneResult, collisionResult] = await Promise.all([projectOne, projectOneCollision]);
	assert.deepEqual(
		projectOneResult.map((session) => session.id),
		["project-1-pending"],
	);
	assert.deepEqual(
		collisionResult.map((session) => session.id),
