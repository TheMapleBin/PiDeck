...
test("AgentManager validates idle state before invoking SessionFileEditor", async () => {
	...
	await assert.rejects(manager.editMessage("agent-1", "agent-1-history-a1", "changed"), /BUSY_STREAMING/);
...
test("AgentManager passes file, active leaf and legacy identity, then loads messages after success", async () => {
	...
	assert.equal(
		commands.some((command) => command.type === "get_entries"),
		false,
	);
...
test("AgentManager does not load messages or report success after editor failure", async () => {
	...
	await assert.rejects(manager.deleteMessage("agent-1", "agent-1-history-a1"), /editor failed/);
...
test("mutatePersistedSessionMessage writes the file without switch_session when idle", async () => {
	...
	const editor = {
		editMessage: async (input) => {
			received.push(["edit", input]);
		},
		deleteMessage: async (input) => {
			received.push(["delete", input]);
		},
		truncateForResend: async (input) => {
			received.push(["resend", input]);
		},
	};
	const { manager, commands } = createHarness(editor);
	manager.agents.clear();
	await manager.mutatePersistedSessionMessage("C:/sessions/session.jsonl", "agent-1-history-a1", "edit", { newText: "changed" });
	await manager.mutatePersistedSessionMessage("C:/sessions/session.jsonl", "agent-1-history-a1", "delete");
	const draft = await manager.mutatePersistedSessionMessage("C:/sessions/session.jsonl", "agent-1-history-a1", "resend");
	...
	try {
		await writeFile(
			...
			[
				...
				{ type: "message", id: "entry-user", parentId: null, message: { role: "user", content: [{ type: "text", text: liveMessage.text }], timestamp: 1234 } },
			]
				.map(JSON.stringify)
				.join("\n") + "\n",
		);
...
test("a persisted entry hint makes the same stopped live message editable", async () => {
	await withStoppedLiveMessage(async (manager, sessionPath, message) => {
		await manager.mutatePersistedSessionMessage(sessionPath, message.id, "edit", {
			entryId: "entry-user",
			newText: "edited with entry anchor",
		});
		assert.equal((await manager.sessionHistoryReader.readMessageByMessageId(sessionPath, "entry-user")).text, "edited with entry anchor");
	});
});
...
test("identity cache merges repeated captures for the same session and stays bounded", () => {
	...
	cache.capture(session, [chatMessage({ id: "live-1", role: "user", text: "q", timestamp: 1, meta: {} })]);
	cache.capture(session, [chatMessage({ id: "agent-2-history-e1", role: "user", text: "q", timestamp: 1, meta: { entryId: "e1" } })]);
...
test("mutatePersistedSessionMessage refuses a live runtime", async () => {
	...
	await assert.rejects(manager.mutatePersistedSessionMessage("C:/sessions/session.jsonl", "agent-1-history-a1", "edit", { newText: "changed" }), /BUSY_GENERIC/);
	assert.equal(called, false);
});
...
test("mutatePersistedSessionMessage delete accepts a message absent from the file (unpersisted turn)", async () => {
	...
	const deleted = await manager.mutatePersistedSessionMessage("C:/sessions/session.jsonl", "agent-1-history-missing", "delete");
