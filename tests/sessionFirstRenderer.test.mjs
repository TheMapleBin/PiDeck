...
const sessionHistoryMutationsSource = readFileSync("src/renderer/src/hooks/useSessionHistoryMutations.ts", "utf8");
...
const drawerSurfaceSource = readFileSync("src/renderer/src/components/workspace/DrawerSurface.tsx", "utf8");
const outlineAtomsSource = readFileSync("src/renderer/src/atoms/session-outline-atoms.ts", "utf8");
...
test("the history drawer uses the lazy Session open path", () => {
	assert.match(drawerSurfaceSource, /onOpenSession=\{/);
});
...
test("first Session send is request-addressed and restores rejected snapshots", () => {
	...
	assert.match(sessionSendSource, /sendingSessionIdsRef\.current\.add\(sourceSessionId\)/);
...
test("the dev workspace toolbar persists for inactive agents and the empty state", () => {
	...
	assert.match(appSource, /<WorkspaceDrawerRail[\s\S]*?id: "files"[\s\S]*?id: "git"[\s\S]*?id: "browser"/);
});
...
test("Session messages and composer render without an active Agent", () => {
	...
	assert.match(sessionViewSource, /bottomComposerVisible && \([\s\S]*<ComposerArea[\s\S]*sessionId=\{sessionId\}/);
