...
test("session status and new-session controls use the shared medium radius", () => {
	...
	const statusBlock = surfaces.slice(surfaces.indexOf(".session-status span"), surfaces.indexOf(".session-status .ctx-chip"));
...
test("model-picker restart must light the SessionView overlay via restartActiveAgent", () => {
	...
	const surfaceStage = readFileSync("src/renderer/src/components/session/SessionSurfaceStage.tsx", "utf8");
	assert.match(surfaceStage, /overlayVisible \? "pointer-events-auto opacity-100" : "pointer-events-none opacity-0"/);
...
test("session header has no bottom border under pane identity row", () => {
	...
	assert.doesNotMatch(header, /chat-header[^"]*border-b/);
});
...
test("split panes expose exit-split expand control on the left", () => {
	...
	assert.match(sessionView, /onExitSplit=\{\s*splitPane \? \(\) => paneServices\.exitSessionSplit\(sessionId\) : undefined\s*\}/);
