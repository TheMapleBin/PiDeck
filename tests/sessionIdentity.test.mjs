...
test("isValidPiSessionFileHead accepts real pi session headers", () => {
	...
	assert.equal(isValidPiSessionFileHead(`${JSON.stringify({ type: "session_info", name: "x", cwd: "C:/repo" })}\n`), true);
});
...
test("treats sibling-dir and parentSessionPath sessions as descendants of the parent", () => {
	...
	const subtree = collectSessionSubtreeIds([linkedChild, siblingOnly, grandchild, unrelated, anonymous], parent);
	// vm 沙箱里的 Array 与宿主 Array 不是同一构造器，deepEqual 会误报；按值比较即可。
	assert.equal([...subtree].sort().join(","), "grandchild,linked-child,parent,sibling-only");
});
