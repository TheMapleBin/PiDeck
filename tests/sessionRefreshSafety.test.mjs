...
// 块边界不能假设缩进宽度：formatter（biome）统一为 tab 后，原先写死的 `\n  \}`
// 会失配并让整组断言报「implementation should be discoverable」。
// 统一用「行首（缩进后）的 } 结束」+ 下一个函数名做边界，两种缩进都成立。
const BLOCK_END = "\\n[\\t ]*\\}\\s*";

function refreshSessionsBlock() {
	const match = projectSync.match(new RegExp(`async function refreshSessions\\(projectId = activeProjectId\\): Promise<SessionSummary\\[\\]> \\{[\\s\\S]*?${BLOCK_END}async function runProjectSessionRefresh`));
	assert.ok(match, "refreshSessions implementation should be discoverable");
	return match[0];
}

function refreshProjectSessionsBlock() {
	const match = projectSync.match(new RegExp(`function refreshProjectSessions\\(projectId: string, silent = false\\): ProjectSessionRefreshPromise \\{[\\s\\S]*?${BLOCK_END}async function refreshProjectTree`));
	assert.ok(match, "refreshProjectSessions implementation should be discoverable");
	return match[0];
}

function runProjectSessionRefreshBlock() {
	const match = projectSync.match(new RegExp(`async function runProjectSessionRefresh\\([\\s\\S]*?${BLOCK_END}function startProjectSessionRefresh`));
...
test("keeps one catalog requester and the legacy adapter", () => {
	...
	assertInOrder(
		...
		[
			...
			"if (!refreshed) return [];",
			".filter((session): session is SessionSummary => Boolean(session))", // 格式化后链式调用可跨行且末行不带分号：只锁调用顺序，不锁行尾标点。
...
test("bounds catalog requests and retains timeout cleanup", () => {
	...
	assert.match(block, /withTimeout\(\s*api\.sessions\.listCatalog\(projectId\),\s*SESSION_REFRESH_TIMEOUT_MS,\s*t\("app\.sessionRefreshTimeout"\),?\s*\)/);
...
test("add project and right-click refresh sync DSH foreign sessions", () => {
	...
	assert.match(projectSync, /async function syncDshForeignSessionsIfEnabled\(\)/, "project sync should own the DSH disk import");
	assertInOrder(projectSync, ["async function refreshProjectTree(project: Project) {", "await syncDshForeignSessionsIfEnabled()", "await refreshProjectSessions(latestProject.id)"], "right-click refresh must import DSH before catalog scan");
	assertInOrder(app, ["async function addProject() {", "await api.projects.add()", "await syncDshForeignSessionsIfEnabled()", "await refreshProjectSessions(project.id)"], "adding a project must import DSH after the path is registered");
