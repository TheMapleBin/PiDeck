...
const { mergeSubagentEntries, applyAsyncSnapshotEntries } = loadTsCommonJs("src/renderer/src/hooks/useSessionSubagents.ts", {
	stubs: {
...
function bridgeLine(agents) {
	return [JSON.stringify({ v: 1, kind: "snapshot", pluginActive: true, agents })];
}

test("merge: bridge 0 defaults do not clobber record toolUses/tokens", () => {
	const { merged } = mergeSubagentEntries([record({ status: "running" })], bridgeLine([{ id: "a1", type: "Explore", description: "d", status: "running", toolUses: 0, tokens: 0 }]));
...
test("merge: bridge positive counts update record while running", () => {
	const { merged } = mergeSubagentEntries([record({ status: "running", toolUses: 2 })], bridgeLine([{ id: "a1", type: "Explore", description: "d", status: "running", toolUses: 7, tokens: 80 }]));
...
test("merge: terminal record status is never overwritten by bridge", () => {
	const { merged } = mergeSubagentEntries([record({ status: "completed", toolUses: 5 })], bridgeLine([{ id: "a1", type: "Explore", description: "d", status: "running", toolUses: 9 }]));
	assert.equal(merged[0].status, "completed");
	assert.equal(merged[0].toolUses, 5);
});

test("merge: no bridge snapshot → pluginActive undefined (three-state)", () => {
	const { merged, pluginActive } = mergeSubagentEntries([record({ id: "a1" })], undefined);
...
test("merge: bridge snapshot with pluginActive false stays false", () => {
	const { merged, pluginActive } = mergeSubagentEntries([], [JSON.stringify({ v: 1, kind: "snapshot", pluginActive: false, agents: [] })]);
	assert.equal(merged.length, 0);
	assert.equal(pluginActive, false);
});

test("merge: bridge-only entries are kept (not yet persisted)", () => {
	const { merged, pluginActive } = mergeSubagentEntries([], bridgeLine([{ id: "b2", type: "code", description: "new", status: "running", toolUses: 1 }]));
	assert.equal(merged.length, 1);
...
test("merge: active entries sort before terminal entries", () => {
	const { merged } = mergeSubagentEntries([record({ id: "done", status: "completed", startedAt: 1700000005000 })], bridgeLine([{ id: "live", type: "code", description: "d", status: "running", startedAt: 1700000001000 }]));
...
test("applyAsyncSnapshot: 快照条目补充同 id 推导条目的 description", () => {
	const merged = applyAsyncSnapshotEntries([derivedAsync()], [widgetAsync()]);
	assert.equal(merged.length, 1);
...
test("applyAsyncSnapshot: 快照状态更新覆盖推导条目，description 仍保留", () => {
	const merged = applyAsyncSnapshotEntries([derivedAsync({ status: "running" })], [widgetAsync({ status: "completed", completedAt: 1700000100000 })]);
	assert.equal(merged[0].status, "completed");
...
test("applyAsyncSnapshot: 快照自带 description 时不被推导条目覆盖", () => {
	const merged = applyAsyncSnapshotEntries([derivedAsync({ description: "推导文本" })], [widgetAsync({ description: "快照文本" })]);
...
test("applyAsyncSnapshot: 空快照原样返回；纯快照条目保留；无描述补充时保持空", () => {
	...
	const noDesc = applyAsyncSnapshotEntries([derivedAsync({ description: "" })], [widgetAsync()]);
