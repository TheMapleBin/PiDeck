...
const { deriveSubagentToolEntries, deriveToolSubagentEntries } = loadTsCommonJs("src/main/pi/derivedSubagents.ts");
const { parseSubagentAsyncSnapshot } = loadTsCommonJs("src/renderer/src/hooks/useSessionSubagents.ts", {
	stubs: {
...
test("derive: isError 结果 → error", () => {
	const entries = deriveSubagentToolEntries([toolCallEntry({ toolCallId: CALL_ID, args: { agent: "worker", task: "T" }, timestamp: T0 }), toolResultEntry({ toolCallId: CALL_ID, text: "boom", timestamp: T1, isError: true })]);
	assert.equal(entries[0].status, "error");
	assert.equal(entries[0].result, "boom");
});

test("derive: 未配对结果的派发保持 running（运行中/被杀由活性降级处理）", () => {
	const entries = deriveSubagentToolEntries([toolCallEntry({ toolCallId: CALL_ID, args: { agent: "worker", task: "T" }, timestamp: T0 })]);
	assert.equal(entries.length, 1);
...
test("derive: action 查询 / 工作流派发不推导；显式后台派发推导为 running", () => {
	...
	const asyncDispatch = deriveSubagentToolEntries([toolCallEntry({ toolCallId: "call_async", args: { agent: "worker", task: "后台任务", async: true }, timestamp: T0 })]);
...
function asyncReceipt({ agent = "worker", id = ASYNC_ID, fanout = false, interactive = true } = {}) {
	...
	const guidance = interactive ? "The async run is detached and running in the background.\nYou are in an interactive session. Return control to the user now." : "The async run is detached. Do not run sleep timers or polling loops just to wait for it.";
...
test("derive: 显式 async:true 派发 + 回执 → 同样重键 asyncId", () => {
	const entries = deriveSubagentToolEntries([toolCallEntry({ toolCallId: "call_async", args: { agent: "scout", task: "T", async: true }, timestamp: T0 }), toolResultEntry({ toolCallId: "call_async", text: asyncReceipt({ agent: "scout", id: "run-2" }), timestamp: T1 })]);
	assert.equal(entries.length, 1);
...
test("derive: 非交互回执（The async run is detached.）同样识别", () => {
	const entries = deriveSubagentToolEntries([toolCallEntry({ toolCallId: CALL_ID, args: { agent: "worker", task: "T" }, timestamp: T0 }), toolResultEntry({ toolCallId: CALL_ID, text: asyncReceipt({ interactive: false }), timestamp: T1 })]);
...
test("derive: 无 asyncId 的回执（如 external-job follow-up）保持 toolCallId 键 running", () => {
	...
	const entries = deriveSubagentToolEntries([toolCallEntry({ toolCallId: CALL_ID, args: { agent: "worker", task: "T" }, timestamp: T0 }), toolResultEntry({ toolCallId: CALL_ID, text: receipt, timestamp: T1 })]);
	assert.equal(entries.length, 1);
...
test("derive: 工作流回执（Async workflow [id]）重键 asyncId；重复回执幂等", () => {
	...
	const orphan = deriveSubagentToolEntries([toolResultEntry({ toolCallId: "call_wf_x", text: wfReceipt, timestamp: T1 })]);
...
test("deriveToolSubagentEntries: acp 与 subagent 两条链并存拼接", () => {
	const entries = deriveToolSubagentEntries([
		toolCallEntry({
			toolCallId: "acp_delegate_1",
			name: "acp_delegate",
			args: { agent: "worker", task: "acp task" },
			timestamp: T0,
		}),
...
test("parseSubagentAsyncSnapshot: 快照行映射为条目（state → status）", () => {
	...
	const entries = parseSubagentAsyncSnapshot(["其他无关行", `PI_SUBAGENT_ASYNC_JSON:${JSON.stringify(snapshot)}`]);
...
test("parseSubagentAsyncSnapshot: 非快照载荷/损坏 JSON/空输入 fail-soft", () => {
	...
	assert.equal(parseSubagentAsyncSnapshot([`PI_SUBAGENT_ASYNC_JSON:${JSON.stringify({ kind: "other" })}`]).length, 0);
});
