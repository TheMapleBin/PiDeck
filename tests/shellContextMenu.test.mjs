...
const { registerShellContextMenu, unregisterShellContextMenu, isShellContextMenuRegistered, SHELL_MENU_KEYS } = compile("src/main/integrations/shellContextMenu.ts");
...
function commandArgOf(key) {
	const call = regCalls.find((c) => c.cmd === "reg" && c.args[0] === "add" && c.args[1] === key && c.args.includes("/ve"));
...
test("command 值带普通引号传给 reg.exe（不手动预转义），Icon 原样写入", async () => {
	...
	assert.equal(commandArgOf(FOLDER_COMMAND), '"C:\\Program Files\\PiDeck\\PiDeck.exe" --open-project "%1"');
	const iconCall = regCalls.find((c) => c.cmd === "reg" && c.args[1] === FOLDER_KEY && c.args.includes("/v") && c.args.includes("Icon"));
...
test("dev 模式（带 app 路径）命令含 electron 与 app 目录两段参数", async () => {
	...
	assert.equal(commandArgOf(FOLDER_COMMAND), '"C:\\electron\\electron.exe" "C:\\dev\\pi-desktop" --open-project "%1"');
});

test("菜单显示名写入文件夹键的默认值", async () => {
	regCalls = [];
	await registerShellContextMenu(EXE, "", "用 PiDeck 打开");
	const call = regCalls.find((c) => c.cmd === "reg" && c.args[1] === FOLDER_KEY && c.args.includes("/ve"));
...
test("查询未注册：任一键 query 失败即 false（短路不查第二个）", async () => {
	...
	const { isShellContextMenuRegistered: checkRegistered } = compile("src/main/integrations/shellContextMenu.ts", failingStub);
	regCalls = [];
