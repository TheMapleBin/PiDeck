...
test("dock keeps the three actions and delegates the homepage link to AboutPopover", () => {
	...
	// TooltipContent 的文本子节点可能被格式化换行：用 \s* 容忍。
	assert.match(dockBlock, /<TooltipContent side="right" sideOffset=\{6\}>\s*\{t\("feedback\.title"\)\}/);
	...
	assert.match(dockBlock, /<TooltipContent side="right" sideOffset=\{6\}>\s*\{themeToggleTitle\}/);
	...
	const dockActions = dockBlock.replace(/<DockItem>\s*<AnnouncementCenter \/>\s*<\/DockItem>/, "");
