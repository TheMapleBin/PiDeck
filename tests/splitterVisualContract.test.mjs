...
test("splitter hover/active stays a single solid line (no gradient double-line)", () => {
	// 高亮规则块：实色单线，位置不变，无渐变
	const block = foundation.match(/\.splitter:hover::before,[\s\S]*?\.splitter\[data-separator="active"\]::before\s*\{([^}]*)\}/);
...
test("layout drag dispatches window resize for floating-layer recalc", () => {
	...
	assert.match(hook, /\.splitter,\s*\.v-splitter,\s*\.session-split-sash,\s*\.session-terminal-splitter/, "track all splitter classes");
