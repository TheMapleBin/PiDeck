...
test("streamdown chat prose density overrides official space-y-4 / heading scale", () => {
	...
	assert.match(streamdownChrome, /\.markdown-body :is\(\.space-y-4\) > :not\(:last-child\):not\(\[data-streamdown\^="heading-"\]\)/);
	// 2026 排版优化（beui 流式观感）：段距 0.55em→0.7em、行高 1.5→1.6、列表/标题留白微增。
	assert.match(streamdownChrome, /margin-block-end:\s*0\.7em/);
	assert.match(streamdownChrome, /\.markdown-body :is\(\.space-y-4\) > \[data-streamdown\^="heading-"\]/);
...
test("streamdown code/table chrome uses faded action controls", () => {
	...
	// 菜单项挤在同一位置文字重叠。
	assert.match(streamdownChrome, /\[data-streamdown="table-wrapper"\] > div:first-child > div > button/);
	assert.doesNotMatch(streamdownChrome, /\[data-streamdown="table-wrapper"\] > div:first-child button/);
	// 顶栏整条 opacity 会建立层叠上下文，鼠标移出后打开的菜单被压到表格内容下面
	// （按 § 取行规则块再断言，避免 regex 越过块边界误伤按钮里的 opacity）
	const rowRuleStart = streamdownChrome.indexOf('[data-streamdown="table-wrapper"] > div:first-child {');
...
test("renderer aliases bare shiki to the fine-grained bundle, never shiki/core subpaths", () => {
	...
	assert.match(vite, /worker:\s*\{[\s\S]*plugins:\s*\(\)\s*=>\s*\[shikiBundleAliasPlugin\(\)\]/, "worker 不继承 renderer.resolve.alias，必须单独挂插件");
	...
	assert.doesNotMatch(bundle, /from "@shikijs\/core"/, "必须用 shiki/core（4.x createBundledHighlighter），不能用顶层 @shikijs/core@2");
	assert.doesNotMatch(bundle, /^export \* from "shiki\/core";/m, "不要把 shiki/core 整包再导出，会和 shorthand 撞名");
});
...
test("streaming overlong guard: plain-text fallback above STREAM_LIGHT_MAX_CHARS", () => {
	...
	// formatter 会给逻辑表达式补外层括号：用 \(? \)? 容忍。
	assert.match(stream, /streamPlain = \(?isStreamingNow && displayText\.length > STREAM_LIGHT_MAX_CHARS/);
	...
	assert.match(stream, /mode="static"/);
	// 三元可能被格式化到同一行：用 \s* 容忍换行。
	assert.match(stream, /resolvedRemarkPlugins = isStreamingNow\s*\?\s*NO_STREAM_REMARK_PLUGINS/);
	// 三元可能被格式化到同一行：用 \s* 容忍换行。
	assert.match(stream, /resolvedRehypePlugins = isStreamingNow\s*\?\s*NO_STREAM_REHYPE_PLUGINS/);
