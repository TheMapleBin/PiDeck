...
const thinkingStep = readFileSync("src/renderer/src/components/session/turn/ThinkingStep.tsx", "utf8");
const timeline = readFileSync("src/renderer/src/components/session/SessionMessageTimeline.tsx", "utf8");
const appUtils = readFileSync("src/renderer/src/components/app/AppUtils.ts", "utf8");
...
test("ThinkingUpdate payload carries stable id + lifecycle fields", () => {
	...
	assert.doesNotMatch(agentTypes.slice(agentTypes.indexOf("export type ThinkingUpdate"), agentTypes.indexOf("export type ComposerAgentMode")), /^\s*thinking:\s*string;/m);
});
...
test("main process: thinking_delta does not upsert; thinking_end does not write messages", () => {
	...
	const messageEndIdx = agentManager.indexOf('if (eventType === "message_end" || eventType === "done" || eventType === "error")', thinkingEndIdx);
