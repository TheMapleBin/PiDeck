...
test("未注入的包名交回 Node 解析（electron 这种需显式桁，不静默造空对象）", () => {
  ...
  assert.throws(
    () => load("src/main/sessions/__missing_dependency_probe__.ts"),
    /ENOENT|Cannot find|no such file/i,
  );
});
...
test("globals 可覆盖 sandbox 全局（自定义 process.platform 等）", () => {
  ...
  const load = createTsSandbox({
    globals: {
      ...
      __record: (value) => { touched.push(value); },
    },
  });
  const mod = load("src/main/sessions/sessionFileSizeCopy.ts");
...
test("解析不到的相对模块报可读错误（含 specifier 与来源文件）", () => {
  const load = createTsSandbox();
  assert.throws(
    () => load("src/main/sessions/__definitely_missing__.ts"),
    /ENOENT|Cannot find|no such file/i,
  );
});
