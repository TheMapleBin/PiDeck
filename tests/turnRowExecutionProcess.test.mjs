...
const thinkingStepSource = readFileSync("src/renderer/src/components/session/turn/ThinkingStep.tsx", "utf8");
const toolStepSource = readFileSync("src/renderer/src/components/session/turn/ToolStep.tsx", "utf8");
const summaryToggleSource = readFileSync("src/renderer/src/components/session/turn/ProcessSummaryToggle.tsx", "utf8");
...
test("renders the run as order-preserving flat display without pulling the last answer to the bottom", () => {
	...
	assert.doesNotMatch(turnRowSource.slice(foldableRenderIndex), /最终回答（始终可见）/);
...
test("issue #130: fold concerns process steps and interim answers, final answer stays inline", () => {
	...
	assert.doesNotMatch(readFileSync("src/renderer/src/i18n/rendererCopy.zh-CN.ts", "utf8"), /executionAnswerCount/);
	assert.match(readFileSync("src/renderer/src/i18n/rendererCopy.zh-CN.ts", "utf8"), /executionInterimCount/);
});
...
test("execution fold renders thinking as collapsed CoT steps", () => {
	...
	assert.doesNotMatch(cards.slice(cards.indexOf("function ThinkingBlock")), /border-dashed border-border-subtle/);
...
test("TurnRow uses custom memo compare so unchanged runs skip re-render", () => {
	...
	// 深度比较入口来自 AppUtils
	assert.match(readFileSync("src/renderer/src/components/app/AppUtils.ts", "utf8"), /export function sameAgentRunForRender/);
});
