...
test("providers-only 文件：弹窗读取零错误（DSH「缺少 probes 数组」黄条回归）", async () => {
	...
	try {
		...
		await writeFile(join(dir, "usage-probes.json"), JSON.stringify({ providers: { deepseek: { enabled: true, timeoutSecs: 10, intervalMinutes: 5 } } }), "utf8");
		const loaded = await loadUsageProbeSettings(dir, "deepseek");
		assert.equal(loaded.config.enabled, true);
		assert.equal(loaded.config.timeoutSecs, 10);
...
test("配置损坏 / 非法条目返回错误而非抛异常", async () => {
	...
	try {
		...
		await writeFile(join(dir, "usage-probes.json"), JSON.stringify({ providers: { bad: { template: "nope" }, ok: { enabled: true } } }), "utf8");
...
test("通用模板：默认复用供应商端点与密钥，覆盖字段优先生效", () => {
	...
	const overridden = buildDeclarativeUsageProbeTemplate("general", { baseUrl: "https://usage.example.com", apiKey: "sk-override" }, endpoint);
...
test("NewAPI 模板：缺访问令牌/用户 ID 报错；齐全时构建鉴权头与积分换算", () => {
	...
	const built = buildDeclarativeUsageProbeTemplate("newapi", { accessToken: "token-123", userId: "114514", baseUrl: "https://relay.example.com" }, endpoint);
...
test("所有内置候选的 templateId 都在类别映射中登记（防漂移）", () => {
	for (const candidate of USAGE_PROBE_CANDIDATES) {
		...
		assert.ok(USAGE_PROBE_CATEGORY_BY_TEMPLATE_ID[candidate.templateId], `内置候选 ${candidate.templateId} 缺少类别登记`);
	}
